...
if($_REQUEST['submit_category'] == 'Create'){
	...
	if(mysql_num_rows($sql_sel_category) == '0'){
		$sql_category_show_order = mysql_fetch_array(mysql_query("SELECT * FROM restaurant_menu_category WHERE restaurant_id = '".$restaurant_id."' ORDER BY show_order DESC LIMIT 0,1"));
		
		$sql_insert_category = mysql_query("INSERT INTO restaurant_menu_category SET restaurant_id = '".$restaurant_id."', category_name = '".$_REQUEST['restaurant_category']."' , show_order = '".($sql_category_show_order['show_order']+1)."'");
		header("location:spl_admin_restaurant_menu.php?catsuccess=1&popup=1");
	}else{
		...
		if(in_array($restaurant_id,$restaurant_arr)){
			header("location:spl_admin_restaurant_menu.php?caterror=1&popup=1");
			
		}else{
			$sql_update_category = mysql_query("UPDATE restaurant_menu_category SET restaurant_id = '".$restaurant_str.",".$restaurant_id."' WHERE category_name = '".($_REQUEST['restaurant_category'])."'");
			header("location:spl_admin_restaurant_menu.php?catsuccess=1&popup=1");
		}
...
if($_REQUEST['del_cat'] == 1)
{
	...
	if($del)
	{
		header("location:spl_admin_restaurant_menu.php?catdelsuccess=1&popup=1");
	}
}

if($_REQUEST['submit_subcategory'] == 'Add'){
	$sql_sel_subcategory = mysql_query("SELECT * FROM restaurant_menu_subcategory WHERE restaurant_id = '".$restaurant_id."' AND category_id = '".$_REQUEST['main_category']."' AND subcategory_name = '".$_REQUEST['sub_category']."' ");

	$sub_cat_num_rows = mysql_num_rows($sql_sel_subcategory);
	
	$sql_show_order = mysql_fetch_array(mysql_query("SELECT * FROM restaurant_menu_subcategory WHERE restaurant_id = '".$restaurant_id."' AND category_id = '".$_REQUEST['main_category']."' ORDER BY show_order DESC LIMIT 0 ,1 "));
	if($sub_cat_num_rows == '0'){
		$sql_insert_subcategory = mysql_query("INSERT INTO restaurant_menu_subcategory SET category_id = '".$_REQUEST['main_category']."' , restaurant_id = '".$restaurant_id."' , subcategory_name = '".$_REQUEST['sub_category']."' , subcategory_desc = '".$_REQUEST['subcategory_description']."' , show_order = '".($sql_show_order['show_order']+1)."'");
		header("location:spl_admin_restaurant_menu.php?subcatsuccess=1&popup=1");
	}else{
		header("location:spl_admin_restaurant_menu.php?subcaterror=1&popup=1");
	}
...
if($_REQUEST['del_subcat'] == '1')
{
	...
	header("location:spl_admin_restaurant_menu.php?subcatdel=1&popup=1&dlt=".$_REQUEST['dlt']."");
}

if($_REQUEST['edit_cat'] == '1')
{
	$select_cat = mysql_fetch_array(mysql_query("SELECT * FROM restaurant_menu_category WHERE id = '".$_REQUEST['cat_edit_id']."'"));
}

if($_REQUEST['edit_submit_category'] == 'Edit')
{
	$check_dup = mysql_num_rows(mysql_query("SELECT id FROM restaurant_menu_category WHERE category_name = '".$_REQUEST['restaurant_category']."' AND id != '".$_REQUEST['cat_edit_id']."'  AND restaurant_edit_id='".$restaurant_id."' "));
	if($check_dup == 0)
	{
		$sql_insert_subcategory = mysql_query("UPDATE restaurant_menu_category SET category_name = '".$_REQUEST['restaurant_category']."' WHERE id = '".$_REQUEST['cat_edit_id']."'");
		header("location:spl_admin_restaurant_menu.php?catedit_success=1&popup=1&edt=".$_REQUEST['pop_tab_id']."");
	}
	else
	{
		header("location:spl_admin_restaurant_menu.php?catedit_error=1&popup=1&edt=".$_REQUEST['pop_tab_id']."");
	}
}
...
if($_REQUEST['submit_subcategory_edit'] == 'Edit')
{
	...
	if($check_dup == 0)
	{
		$sql_insert_subcategory = mysql_query("UPDATE restaurant_menu_subcategory SET category_id = '".$_REQUEST['main_category']."' , restaurant_id = '".$restaurant_id."' , subcategory_name = '".$_REQUEST['sub_category']."' , subcategory_desc = '".$_REQUEST['subcategory_description']."' WHERE id = '".$_REQUEST['edit_id']."'");
		header("location:spl_admin_restaurant_menu.php?subedit_success=1&popup=1&edt=".$_REQUEST['edt']."");
	}
	else
	{
		header("location:spl_admin_restaurant_menu.php?subedit_error=1&popup=1&edt=".$_REQUEST['edt']."");
	}
...
if($_REQUEST['del_item'] == '1')
{
	...
	header("location:spl_admin_restaurant_menu.php?itemdel=1&cat_id=".$sql_sel['category_id']."&sub_cat=".$sql_sel['sub_category_id']."&open_div=1");
}
...
if($_REQUEST['sub_button'] == "Copy Menu")
{
	...
	header("location:spl_admin_restaurant_menu.php?success=3");
...
if($_REQUEST['submit'] == 'Save & Continue'){	
	...
	foreach($_POST['countmainmenudiv'] as $countDiv){
		...
		foreach($_POST['countmenudiv_'.$countDiv] as $countmenudiv){
			...
			$menu_item_whats_good 	  = $_POST['menu_item_whats_good_'.$countDiv.'_'.$countmenudiv.''];
			$menu_item_spicy 	  	  = $_POST['menu_item_spicy_'.$countDiv.'_'.$countmenudiv.''];
			$menu_item_veggie 	      = $_POST['menu_item_veggie_'.$countDiv.'_'.$countmenudiv.''];
			$menu_item_healthy 	      = $_POST['menu_item_healthy_'.$countDiv.'_'.$countmenudiv.''];			
	...
	header("location:spl_admin_restaurant_menu.php?item_add=1&anc=".$_REQUEST['hid_anc_id']."");
...
if($_REQUEST['submit_item'] == 'Save & Continue')
{
	...
	/*$data['whats_good'] = $_REQUEST['whats_good'];
	$data['spicy'] = $_REQUEST['spicy'];
	$data['veggie'] = $_REQUEST['veggie'];
	$data['healthy'] = $_REQUEST['healthy'];*/
	...
	foreach($data['item_id'] as $item_id)
	{
		...
		$data['whats_good'] = $_REQUEST['whats_good_'.$item_id.'_'.$i];
		$data['spicy'] = $_REQUEST['spicy_'.$item_id.'_'.$i];
		$data['veggie'] = $_REQUEST['veggie_'.$item_id.'_'.$i];
		$data['healthy'] = $_REQUEST['healthy_'.$item_id.'_'.$i];
			...
			$update = "UPDATE restaurant_menu_item SET menu_name = '".$data['menu_item_name'][$i]."' , price = '".$data['menu_item_price'][$i]."' , description = '".$data['menu_item_description'][$i]."' ,whats_good = '".$data['whats_good']."' , spicy = '".$data['spicy']."' , veggie = '".$data['veggie']."' , healthy = '".$data['healthy']."'";
	...
	/*if($update)
	{*/
		header("location:spl_admin_restaurant_menu.php?item_edit=1&open_div=1&cat_id=".$sql_menu_item['category_id']."&sub_cat=".$subcat."");
		exit;
	/*}*/
}
...
if($_REQUEST['del_ins']!='')
{
	...
	header("location:spl_admin_restaurant_menu.php?insdelitm=1&inspop=1&menu_id=".$_REQUEST['menu_id']."&cat_id=".$sql_menu_item['category_id']."&sub_cat=".$sql_menu_item['sub_category_id']."");
}
...
if($_REQUEST['sub_ins_button'] = "Copy Special Instructions")
{
	...
	{ 
		...
		$ins = mysql_query("INSERT INTO restaurant_menu_special_instruction SET restaurant_id = '".$restaurant_id."' , menu_id = '".$menu_id."' , special_instruction = '".$sel_menu['special_instruction']."'");
		...
		{
			$update = mysql_query("INSERT INTO restaurant_menu_item_special_instruction SET restaurant_id = '".$restaurant_id."' , menu_id = '".$menu_id."' , special_ins_id = '".$last_id."' , title = '".$row['title']."' , price = '".$row['price']."' , show_order = '".$i."' ");
			$i++;
	...
	if($update)
	{
		header("location:spl_admin_restaurant_menu.php?copyins=1");
	}
}
...
if($_REQUEST['edit_spl_ins'] == 'Save & Continue'){
	...
	while($array_sel_ins = mysql_fetch_array($sql_sel_spl_ins)){
		...
		foreach($_REQUEST['countoptions'.$array_sel_ins['id']] as $countmainins){
			...
			$sql_get_show_order = mysql_fetch_array(mysql_query("SELECT * FROM restaurant_menu_item_special_instruction WHERE special_ins_id = '".$array_sel_ins['id']."' AND restaurant_id = '".$restaurant_id."' AND menu_id = '".$_REQUEST['menu_id']."' ORDER BY show_order DESC LIMIT 0,1"));
			
			$sql_insert_option = mysql_query("INSERT INTO restaurant_menu_item_special_instruction SET title = '".$title."' , price = '".$price."' , special_ins_id = '".$array_sel_ins['id']."' , restaurant_id = '".$restaurant_id."', menu_id = '".$_REQUEST['menu_id']."' , show_order = '".($sql_get_show_order['show_order']+1)."'");
		}
	...
	foreach($_REQUEST['spl_arr'.$_REQUEST['menu_id']] as $spl_ins_id){
		$sql_insert_ins = mysql_query("INSERT INTO restaurant_menu_special_instruction SET restaurant_id = '".$restaurant_id."' , menu_id = '".$_REQUEST['menu_id']."' , special_instruction = '".$_REQUEST['spl_ins_txt_ajax'.$spl_ins_id]."'");
		$last_id = mysql_insert_id();
		
		$inc = 1;
		foreach($_REQUEST['opt_arr'.$_REQUEST['menu_id'].'_'.$spl_ins_id] as $opt_ins_id){
			$sql_insert_opt = mysql_query("INSERT INTO restaurant_menu_item_special_instruction SET title = '".$_REQUEST['ins_title_ajax_'.$spl_ins_id.'_'.$opt_ins_id]."' , price = '".$_REQUEST['ins_price_ajax_'.$spl_ins_id.'_'.$opt_ins_id]."' , special_ins_id = '".$last_id."' , restaurant_id = '".$restaurant_id."', menu_id = '".$_REQUEST['menu_id']."' , show_order = '".$inc."'");
		$inc++;
		}
	}
	
	header("location:spl_admin_restaurant_menu.php?insdelitm=1&inspop=1&menu_id=".$_REQUEST['menu_id']."&cat_id=".$sql_menu_item['category_id']."&sub_cat=".$sql_menu_item['sub_category_id']."&edit_succ_ins=1");
}


if($_REQUEST['submit_cat_time'] == "Submit")
{
	$data['mon_from'] = $_REQUEST['mon_from'];
	$data['mon_to'] = $_REQUEST['mon_to'];
	$data['tue_from'] = $_REQUEST['tue_from'];
	$data['tue_to'] = $_REQUEST['tue_to'];
	$data['wed_from'] = $_REQUEST['wed_from'];
	$data['wed_to'] = $_REQUEST['wed_to'];
	$data['thu_from'] = $_REQUEST['thu_from'];
	$data['thu_to'] = $_REQUEST['thu_to'];
	$data['fri_from'] = $_REQUEST['fri_from'];
	$data['fri_to'] = $_REQUEST['fri_to'];
	$data['sat_from'] = $_REQUEST['sat_from'];
	$data['sat_to'] = $_REQUEST['sat_to'];
	$data['sun_from'] = $_REQUEST['sun_from'];
	$data['sun_to'] = $_REQUEST['sun_to'];
	
	$data['cat_id'] = $_REQUEST['hid_cat_time_id'];
	
	$update = mysql_query("UPDATE restaurant_menu_category SET category_monday_from = '".$data['mon_from']."' , category_monday_to = '".$data['mon_to']."' , category_tuesday_from = '".$data['tue_from']."' , category_tuesday_to = '".$data['tue_to']."' , category_wednesday_from = '".$data['wed_from']."' , category_wednesday_to = '".$data['wed_to']."' , category_thursday_from = '".$data['thu_from']."' , category_thursday_to = '".$data['thu_to']."' , category_friday_from = '".$data['fri_from']."' , category_friday_to = '".$data['fri_to']."' , category_saturday_from = '".$data['sat_from']."' , category_saturday_to = '".$data['sat_to']."' , category_sunday_from = '".$data['sun_from']."' , category_sunday_to = '".$data['sun_to']."' WHERE id = '".$data['cat_id']."' ");
	
	if($update)
	{
		header("location:spl_admin_restaurant_menu.php?cattimesuccess=1&popup=1");
	}
	
	
}

if($_REQUEST['submit_sub_cat_time'] == "Submit")
{
	$data['sub_cat_mon_from'] = $_REQUEST['sub_cat_mon_from'];
	$data['sub_cat_mon_to'] = $_REQUEST['sub_cat_mon_to'];
	$data['sub_cat_tue_from'] = $_REQUEST['sub_cat_tue_from'];
	$data['sub_cat_tue_to'] = $_REQUEST['sub_cat_tue_to'];
	$data['sub_cat_wed_from'] = $_REQUEST['sub_cat_wed_from'];
	$data['sub_cat_wed_to'] = $_REQUEST['sub_cat_wed_to'];
	$data['sub_cat_thu_from'] = $_REQUEST['sub_cat_thu_from'];
	$data['sub_cat_thu_to'] = $_REQUEST['sub_cat_thu_to'];
	$data['sub_cat_fri_from'] = $_REQUEST['sub_cat_fri_from'];
	$data['sub_cat_fri_to'] = $_REQUEST['sub_cat_fri_to'];
	$data['sub_cat_sat_from'] = $_REQUEST['sub_cat_sat_from'];
	$data['sub_cat_sat_to'] = $_REQUEST['sub_cat_sat_to'];
	$data['sub_cat_sun_from'] = $_REQUEST['sub_cat_sun_from'];
	$data['sub_cat_sun_to'] = $_REQUEST['sub_cat_sun_to'];
	
	$data['sub_cat_id'] = $_REQUEST['hid_sub_cat_time_id'];
	
	$update = mysql_query("UPDATE restaurant_menu_subcategory SET sub_category_monday_from = '".$data['sub_cat_mon_from']."' , sub_category_monday_to = '".$data['sub_cat_mon_to']."' , sub_category_tuesday_from = '".$data['sub_cat_tue_from']."' , sub_category_tuesday_to = '".$data['sub_cat_tue_to']."' ,  	sub_category_wednesday_from = '".$data['sub_cat_wed_from']."' , sub_category_wednesday_to = '".$data['sub_cat_wed_to']."' , sub_category_thursday_from = '".$data['sub_cat_thu_from']."' , sub_category_thursday_to = '".$data['sub_cat_thu_to']."' , sub_category_friday_from = '".$data['sub_cat_fri_from']."' , sub_category_friday_to = '".$data['sub_cat_fri_to']."' , sub_category_saturday_from = '".$data['sub_cat_sat_from']."' , sub_category_saturday_to = '".$data['sub_cat_sat_to']."' , sub_category_sunday_from = '".$data['sub_cat_sun_from']."' , sub_category_sunday_to = '".$data['sub_cat_sun_to']."' WHERE id = '".$data['sub_cat_id']."' ");
	
	if($update)
	{
		header("location:spl_admin_restaurant_menu.php?subcattimesuccess=1&popup=1");
	}
...
function add_options(id,spl_ins_id){
	$.ajax({
		...
		success : function( data, textStatus, jqXHR){
			...
			document.getElementById('add_options_'+spl_ins_id).value=parseInt(id)+parseInt(1);
		},
		/*complete : function( jqXHR, textStatus){
			alert(3);
		},*/
...
<script type="text/javascript">
function open_cat_timing_div(id)
{
	$("#nw_pop").hide();
	$("#cat_timing_div"+id).show();
	$("#fade1").show();
}

function close_cat_timing_div(id)
{
	$("#cat_timing_div"+id).hide();
	$("#nw_pop").show();
	$("#fade1").show();
}

function open_subcat_timing_div(id)
{
	$("#nw_pop").hide();
	$("#subcat_timing_div"+id).show();
	$("#fade1").show();
}

function close_subcat_timing_div(id)
{
	$("#subcat_timing_div"+id).hide();
	$("#nw_pop").show();
	$("#fade1").show();
}
</script>
...
<div class="restaurant_body_cont menu_body_cont">
...
<h1>Menu Process - <br><a href="javascript:void(0);" class="button4 menu-btn" onClick="open_cat_pop_up();">Add & Manage Menu Category </a> </h1>
...
<div id="nw_pop" class="white_content xslg">
	<a href="javascript:void(0);" class="close" onClick="close_cat_pop_up();"><img width="16" height="16" src="images/cross-blue.png"> </a>
   ...
   <?php if($_REQUEST['cattimesuccess'] == '1'){ ?> 
   <p style="color:#090; text-align:center;">Category Time Added Successfully.</p>
   <?php } ?>
   
   <?php if($_REQUEST['subcattimesuccess'] == '1'){ ?> 
   <p style="color:#090; text-align:center;">Sub Category Time Added Successfully.</p>
   <?php } ?>
   ...
   <?php } ?>
    ...
    <input type="hidden" name="restaurant_edit_id" id="restaurant_edit_id" value="<?php echo $restaurant_id; ?>">
        <div class="main-cat">
            <label>Create Main Category <em>*</em></label>
            <div class="fildrow">
                <input type="text" name="restaurant_category" id="restaurant_category" style="width:50%;" value="<?php if($select_cat['category_name'] !='')echo $select_cat['category_name']; ?>" />
                <input type="hidden" name="pop_tab_id" id="pop_tab_id" />
                <?php if($_REQUEST['cat_edit_id'] == '') {?>
                <input type="submit" name="submit_category" id="submit_category" value="Create" class="btn"  onClick="return validate_category();"/>
                <?php }else{ ?>
                <input type="submit" name="edit_submit_category" id="edit_submit_category" value="Edit" class="btn"  onClick="return validate_category();"/>
                <?php } ?>
            </div>
            <div class="clear"></div>
        </div>
				...
				<ul>
					<li>
                        <select name="main_category" id="main_category" class="restaurant_list" style="width:170px; margin:0px;">
                            ...
                            <?php $sql_sel_category = mysql_query("SELECT * FROM restaurant_menu_category WHERE restaurant_id IN (".$restaurant_id.") ORDER BY category_name ASC ");
	...
	<?php
	$sql_cat = mysql_query("SELECT * FROM restaurant_menu_category WHERE restaurant_id IN (".$restaurant_id.") ORDER BY category_name ASC ");
	$sql_cat_single = mysql_query("SELECT * FROM restaurant_menu_category WHERE restaurant_id = ".$restaurant_id." ORDER BY show_order");
	?>
    
    <div style="height:300px; overflow:auto;">
    
    <div class="full-width pop-table" id="res_menu_cat_<?php echo $restaurant_id; ?>" style="height: 210px;">
        <table cellpadding="0" cellspacing="0" class="blue-table"> 
           ...
           <?php 
		   $inc_cat = 1;
		   while($row_cat_single = mysql_fetch_assoc($sql_cat_single)) { 
		   $sql_subcat2 = mysql_query("SELECT * FROM restaurant_menu_subcategory WHERE category_id = '".$row_cat_single['id']."' AND restaurant_id = '".$restaurant_id."' ORDER BY show_order ");
			
			$min_order_id = mysql_fetch_array(mysql_query("SELECT MIN(show_order) as min_id FROM restaurant_menu_category WHERE restaurant_id = '".$restaurant_id."' "));
                $max_order_id = mysql_fetch_array(mysql_query("SELECT MAX(show_order) as max_id FROM restaurant_menu_category WHERE  restaurant_id = '".$restaurant_id."' "));
                
			
			?>
            <tr>
                <td align="center"><?php echo $inc_cat; ?></td>
                <td><?php echo $row_cat_single['category_name']; ?></td>
                <td align="center"><?php if($min_order_id['min_id']!= $row_cat_single['show_order']){ ?>
                <a href="order_change_up_res_cat.php?res_id=<?php echo $restaurant_id; ?>&show_order=<?php echo $row_cat_single['show_order']; ?>&id=<?php echo $row_cat_single['id']; ?>&pg_name=spl_res"> <img src="images/icon-arrow-up-b-16.png" alt="Order Up" title="Order Up" align="absmiddle" /></a>
            <?php } ?>
            <?php if($max_order_id['max_id']!= $row_cat_single['show_order']){ ?>
                <a href="order_change_down_res_cat.php?res_id=<?php echo $restaurant_id; ?>&show_order=<?php echo $row_cat_single['show_order']; ?>&id=<?php echo $row_cat_single['id']; ?>&pg_name=spl_res"> <img src="images/icon-arrow-down-b-16.png" alt="Order Down" title="Order Down" align="absmiddle" /></a>
            <?php } ?>
                            </td>
                <td align="center">
                <a href="spl_admin_restaurant_menu.php?restaurant_edit_id=<?php echo $restaurant_id; ?>&cat_edit_id=<?php echo $row_cat_single['id'];?>&edit_cat=1&popup=1&cat_edt=<?php echo $inc_cat; ?>"> <img src="images/pen_edit_write_-16.png" alt="Edit" title="Edit" align="absmiddle"></a>
                
                <a href="javascript:void(0);" onClick="open_cat_timing_div('<?php echo $row_cat_single['id'];?>');"><img src="images/time.png" width="16" height="16" title="Add Timing"></a>
                
                <?php
                if(mysql_num_rows($sql_subcat2) == 0)
				{
				?>
                	<a href="spl_admin_restaurant_menu.php?restaurant_edit_id=<?php echo $restaurant_id; ?>&del_cat_id=<?php echo $row_cat_single['id']; ?>&del_cat=1" onClick="return confirm('Are you sure to Delete this Category?');"> <img src="images/DeleteRed.png" alt="Delete" title="Delete" align="absmiddle"></a>
                <?php
				}
				else
				{
				?>
                	<a href="javascript:void(0);" alt="Delete" title="Delete" align="absmiddle"><img src="images/delete_disabled.png" alt="Delete" title="Delete Disabled as Subcategory exists under this Category" align="absmiddle"></a>
                <?php
				}
				?>
                </td>
	...
	<div class="restaurant_nav ">
		...
		{
			$sql_subcat1 = mysql_query("SELECT * FROM restaurant_menu_subcategory WHERE category_id = '".$row_cat['id']."' AND restaurant_id = '".$restaurant_id."' ORDER BY show_order ");
		...
		?>
		    <li class="li_cls"><?php /*?><a href="spl_admin_restaurant_menu.php?del_cat_id=<?php echo $row_cat['id']; ?>&del_cat=1" onClick="return confirm('Are you sure to Delete this Category?');"><img class="del_img" src="<?php echo $image_path; ?>" width="10" style="display:none" alt="Delete" title="<?php echo $title; ?>" align="absmiddle"></a><?php */?><a href="javascript:void(0);" id="cat_anc<?php echo $j; ?>" onClick="open_cat_div('<?php echo $j; ?>');"><?php echo $row_cat['category_name']; ?></a></li>
        <?php
		$j++;
		}
		?>
    ...
    <?php
	$sql_cat1 = mysql_query("SELECT * FROM restaurant_menu_category WHERE restaurant_id IN (".$restaurant_id.") ORDER BY show_order ");
	$jj = 1;
	...
	<div class="full-width pop-table" id="cat_desc<?php echo $jj; ?>" style="display:none;">
        <table cellpadding="0" cellspacing="0" class="blue-table"> 
            ...
            <?php
			$sql_subcat = mysql_query("SELECT * FROM restaurant_menu_subcategory WHERE category_id = '".$row_cat1['id']."' AND restaurant_id = '".$restaurant_id."' ORDER BY show_order ");
			$subcat_num_rows = mysql_num_rows($sql_subcat);
			if($subcat_num_rows > 0)
			{
				$min_order_id = mysql_fetch_array(mysql_query("SELECT MIN(show_order) as min_id FROM restaurant_menu_subcategory WHERE category_id = '".$row_cat1['id']."' AND restaurant_id = '".$restaurant_id."' "));
                $max_order_id = mysql_fetch_array(mysql_query("SELECT MAX(show_order) as max_id FROM restaurant_menu_subcategory WHERE category_id = '".$row_cat1['id']."' AND restaurant_id = '".$restaurant_id."' "));
			$sl = 1;
			...
			?>
				<tr>
                    ...
                    <?php if($min_order_id['min_id']!= $row_subcat['show_order']){ ?>
						<a href="order_change_up_res_sub_cat.php?cat_id=<?php echo $row_subcat['category_id']; ?>&res_id=<?php echo $restaurant_id; ?>&show_order=<?php echo $row_subcat['show_order']; ?>&id=<?php echo $row_subcat['id']; ?>&sh_odr=<?php echo $jj; ?>&pg_name=spl_res"> <img src="images/icon-arrow-up-b-16.png" alt="Order Up" title="Order Up" align="absmiddle" /></a>
                    <?php } ?>
                    <?php if($max_order_id['max_id']!= $row_subcat['show_order']){ ?>
						<a href="order_change_down_res_sub_cat.php?cat_id=<?php echo $row_subcat['category_id']; ?>&res_id=<?php echo $restaurant_id; ?>&show_order=<?php echo $row_subcat['show_order']; ?>&id=<?php echo $row_subcat['id']; ?>&sh_odr=<?php echo $jj; ?>&pg_name=spl_res"> <img src="images/icon-arrow-down-b-16.png" alt="Order Down" title="Order Down" align="absmiddle" /></a>
                    <?php } ?>
					</td>
					<td align="center">
						<a href="spl_admin_restaurant_menu.php?edit_id=<?php echo $row_subcat['id'];?>&edit_subcat=1&popup=1&edt=<?php echo $jj; ?>"> <img src="images/pen_edit_write_-16.png" alt="Edit" title="Edit" align="absmiddle" /></a>
                        
                        <a href="javascript:void(0);" onClick="open_subcat_timing_div('<?php echo $row_subcat['id'];?>');"><img src="images/time.png" width="16" height="16" title="Add Timing"></a>
                        
						<a href="spl_admin_restaurant_menu.php?del_id=<?php echo $row_subcat['id'];?>&del_subcat=1&dlt=<?php echo $jj; ?>" onClick="return confirm('Are you sure to Delete this Sub Category?')"> <img src="images/DeleteRed.png" alt="Delete" title="Delete" align="absmiddle" /></a>
					</td>
				</tr>
			<?php
			$sl++;
...
<script>
  $(function() {
	...
	/*$('#mon_to').timepicker({ 'timeFormat': 'h:i A' });
	$('#tue_from').timepicker({ 'timeFormat': 'h:i A' });
	$('#tue_to').timepicker({ 'timeFormat': 'h:i A' });
	$('#wed_from').timepicker({ 'timeFormat': 'h:i A' });
	$('#wed_to').timepicker({ 'timeFormat': 'h:i A' });
	$('#thu_from').timepicker({ 'timeFormat': 'h:i A' });
	$('#thu_to').timepicker({ 'timeFormat': 'h:i A' });
	$('#fri_from').timepicker({ 'timeFormat': 'h:i A' });
	$('#fri_to').timepicker({ 'timeFormat': 'h:i A' });
	$('#sat_from').timepicker({ 'timeFormat': 'h:i A' });
	$('#sat_to').timepicker({ 'timeFormat': 'h:i A' });
	$('#sun_from').timepicker({ 'timeFormat': 'h:i A' });
	$('#sun_to').timepicker({ 'timeFormat': 'h:i A' });*/
  });
</script>
<script type="text/javascript">
...
function check_tue_buss_from(val)
{
	var bussto = Converttimeformat($("#tue_from").val());
...
function check_tue_buss_to(val)
{
	var bussfrom = Converttimeformat($("#tue_to").val());
	var value = Converttimeformat(val);
...
<?php 
$sql_cat_single1 = mysql_query("SELECT * FROM restaurant_menu_category WHERE restaurant_id = ".$restaurant_id." ORDER BY show_order");
while($row_cat_single1 = mysql_fetch_assoc($sql_cat_single1)) {?>
<div id="cat_timing_div<?php echo $row_cat_single1['id'];?>" style="display:none;" class="white_content"  >
<div class="close close-new" onClick="close_cat_timing_div('<?php echo $row_cat_single1['id'];?>');"><a href = "javascript:void(0);"> </a> </div>
<div class="l-contnt up-contnt"> 
<span style="border-bottom:1px dashed #eee;display: block;"><h2>Add <?php echo $row_cat_single1['category_name']; ?> Availability Hours</h2></span>
<div class="clear"></div>
<form name="cat_time_form" id="cat_time_form" method="post" action="" enctype="multipart/form-data">
<p>Category Hours (Monday):</p>
<input name="mon_from" id="mon_from" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_cat_single1['category_monday_from']; ?>">
<span style="color:#888888;">to</span>
<input name="mon_to" id="mon_to" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_cat_single1['category_monday_to']; ?>">

<div class="clear"></div>
<p>Category Hours (Tuesday):</p>
<input name="tue_from" id="tue_from" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_cat_single1['category_tuesday_from']; ?>" onBlur="check_tue_buss_from(this.value);">
<span style="color:#888888;">to</span>
<input name="tue_to" id="tue_to" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_cat_single1['category_tuesday_to']; ?>" onBlur="check_tue_buss_to(this.value)">

<div class="clear"></div>
<p>Category Hours (Wednesday):</p>
<input name="wed_from" id="wed_from" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_cat_single1['category_wednesday_from']; ?>">
<span style="color:#888888;">to</span>
<input name="wed_to" id="wed_to" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_cat_single1['category_wednesday_to']; ?>">

<div class="clear"></div>
<p>Category Hours (Thursday):</p>
<input name="thu_from" id="thu_from" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_cat_single1['category_thursday_from']; ?>">
<span style="color:#888888;">to</span>
<input name="thu_to" id="thu_to" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_cat_single1['category_thursday_to']; ?>">

<div class="clear"></div>
<p>Category Hours (Friday):</p>
<input name="fri_from" id="fri_from" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_cat_single1['category_friday_from']; ?>">
<span style="color:#888888;">to</span>
<input name="fri_to" id="fri_to" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_cat_single1['category_friday_to']; ?>">

<div class="clear"></div>
<p>Category Hours (Saturday):</p>
<input name="sat_from" id="sat_from" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_cat_single1['category_saturday_from']; ?>">
<span style="color:#888888;">to</span>
<input name="sat_to" id="sat_to" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_cat_single1['category_saturday_to']; ?>">

<div class="clear"></div>
<p>Category Hours (Sunday):</p>
<input name="sun_from" id="sun_from" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_cat_single1['category_sunday_from']; ?>">
<span style="color:#888888;">to</span>
<input name="sun_to" id="sun_to" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_cat_single1['category_sunday_to']; ?>">

<div class="clear"></div>
<input type="hidden" name="hid_cat_time_id" id="hid_cat_time_id" value="<?php echo $row_cat_single1['id'];?>" />
<p><input type="submit" class="button4" name="submit_cat_time" id="submit_cat_time" value="Submit" /></p>
</form>
</div>
</div>
<?php } ?>
...
<?php 
$sql_cat_single2 = mysql_query("SELECT * FROM restaurant_menu_category WHERE restaurant_id = ".$_REQUEST['restaurant_edit_id']." ORDER BY show_order");
while($row_cat_single2 = mysql_fetch_assoc($sql_cat_single2)) {
$sql_subcat1 = mysql_query("SELECT * FROM restaurant_menu_subcategory WHERE category_id = '".$row_cat_single2['id']."' AND restaurant_id = '".$_REQUEST['restaurant_edit_id']."' ORDER BY show_order ");
while($row_subcat1 = mysql_fetch_assoc($sql_subcat1)) { ?>
<div id="subcat_timing_div<?php echo $row_subcat1['id'];?>" style="display:none;" class="white_content"  >
<div class="close close-new" onClick="close_subcat_timing_div('<?php echo $row_subcat1['id'];?>');"><a href = "javascript:void(0);"> </a> </div>
<div class="l-contnt up-contnt"> 
<span style="border-bottom:1px dashed #eee;display: block;"><h2>Add <?php echo $row_subcat1['subcategory_name']; ?> Availability Hours</h2></span>
<form name="subcat_time_form" id="subcat_time_form" method="post" action="" enctype="multipart/form-data">
<p>Subcategory Hours (Monday):</p>
<input name="sub_cat_mon_from" id="sub_cat_mon_from" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_subcat1['sub_category_monday_from']; ?>">
<span style="color:#888888;">to</span>
<input name="sub_cat_mon_to" id="sub_cat_mon_to" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_subcat1['sub_category_monday_to']; ?>">
<p></p>
<p>Subcategory Hours (Tuesday):</p>
<input name="sub_cat_tue_from" id="sub_cat_tue_from" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_subcat1['sub_category_tuesday_from']; ?>" onBlur="check_tue_buss_from(this.value);">
<span style="color:#888888;">to</span>
<input name="sub_cat_tue_to" id="sub_cat_tue_to" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_subcat1['sub_category_tuesday_to']; ?>" onBlur="check_tue_buss_to(this.value)">
<p></p>
<p>Subcategory Hours (Wednesday):</p>
<input name="sub_cat_wed_from" id="sub_cat_wed_from" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_subcat1['sub_category_wednesday_from']; ?>">
<span style="color:#888888;">to</span>
<input name="sub_cat_wed_to" id="sub_cat_wed_to" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_subcat1['sub_category_wednesday_to']; ?>">
<p></p>
<p>Subcategory Hours (Thursday):</p>
<input name="sub_cat_thu_from" id="sub_cat_thu_from" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_subcat1['sub_category_thursday_from']; ?>">
<span style="color:#888888;">to</span>
<input name="sub_cat_thu_to" id="sub_cat_thu_to" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_subcat1['sub_category_thursday_to']; ?>">
<p></p>
<p>Subcategory Hours (Friday):</p>
<input name="sub_cat_fri_from" id="sub_cat_fri_from" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_subcat1['sub_category_friday_from']; ?>">
<span style="color:#888888;">to</span>
<input name="sub_cat_fri_to" id="sub_cat_fri_to" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_subcat1['sub_category_friday_to']; ?>">
<p></p>
<p>Subcategory Hours (Saturday):</p>
<input name="sub_cat_sat_from" id="sub_cat_sat_from" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_subcat1['sub_category_saturday_from']; ?>">
<span style="color:#888888;">to</span>
<input name="sub_cat_sat_to" id="sub_cat_sat_to" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_subcat1['sub_category_saturday_to']; ?>">
<p></p>
<p>Subcategory Hours (Sunday):</p>
<input name="sub_cat_sun_from" id="sub_cat_sun_from" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_subcat1['sub_category_sunday_from']; ?>">
<span style="color:#888888;">to</span>
<input name="sub_cat_sun_to" id="sub_cat_sun_to" type="text" class="restaurant time_pick " style="width:93px;"  autocomplete="off" value="<?php echo $row_subcat1['sub_category_sunday_to']; ?>">
<p></p>
<input type="hidden" name="hid_sub_cat_time_id" id="hid_sub_cat_time_id" value="<?php echo $row_subcat1['id'];?>" />
<p><input type="submit" class="button4" name="submit_sub_cat_time" id="submit_sub_cat_time" value="Submit" /></p>
</form>
</div>
</div>
<?php } } ?>
...
<div class="new-res-fild">
	<div class="catsec">
		...
		<select name="menu_main_category1" id="menu_main_category1" class="restaurant_list" onChange="getSubcat(this.value,'1');">
            ...
            <?php $sql_get_restaurant_category = mysql_query("SELECT * FROM restaurant_menu_category WHERE restaurant_id IN (".$restaurant_id.") ORDER BY category_name ASC");
    ...
    <?php /*?><input type="hidden" name="countsplinsdiv[]" value="1" class="webcampics"><?php */?>
        <div class="new-res-fild">
            ...
            <div class="addmenu-fild btnsec">
             ...
             <div class="clear"></div>
            <div class="addmenu-fild">
                <input type="checkbox" name="menu_item_whats_good_1_1" id="menu_item_whats_good_1_1" value="1" style="height:12px; width: 15px;"> What's Good
            </div>
            <div class="addmenu-fild">
                <input type="checkbox" name="menu_item_spicy_1_1" id="menu_item_spicy_1_1" value="1" style="height:12px; width: 15px;"> Spicy
            </div>
            <div class="addmenu-fild">
                <input type="checkbox" name="menu_item_veggie_1_1" id="menu_item_veggie_1_1" value="1" style="height:12px; width: 15px;"> Veggie
            </div>
            <div class="addmenu-fild">
                <input type="checkbox" name="menu_item_healthy_1_1" id="menu_item_healthy_1_1" value="1" style="height:12px; width: 15px;"> Healthy
            </div>
        </div> 
...
<div class="add_item" style="float:left;"><a href="javascript:void(0);" onClick="add_cell(document.getElementById('item_id').value,'<?php echo $restaurant_id; ?>')" id="item_focus"><img src="images/add_item.png"  style="margin-left:0;"/></a></div>

<input type="hidden" id="hid_anc_id" name="hid_anc_id" />

<div  style="float:right;">

<input class="button4 menu_submit" type="submit" value="Save & Continue" name="submit"  style="margin-top:0;"></div>
...
<?php
	$sql_cat_new = mysql_query("SELECT * FROM restaurant_menu_category WHERE restaurant_id IN (".$restaurant_id.") ORDER BY show_order "); 
	...
	<div class="restaurant_nav ">
		...
		{
		?>
		    <li><a href="javascript:void(0);" id="cat_anc_new<?php echo $k; ?>" onClick="open_cat_btm_div('<?php echo $row_cat_new['id']; ?>');"><?php echo $row_cat_new['category_name']; ?></a></li>
        <?php
		$k++;
		}
		?>
    ...
    <?php
	$sql_cat_new_div = mysql_query("SELECT * FROM restaurant_menu_category WHERE restaurant_id IN (".$restaurant_id.") ORDER BY category_name ASC ");
	$l = 1;
	while($row_cat_new_div = mysql_fetch_assoc($sql_cat_new_div))
	{ 
		$sql_subcat_new = mysql_query("SELECT * FROM restaurant_menu_subcategory WHERE category_id = '".$row_cat_new_div['id']."' AND restaurant_id = '".$restaurant_id."' ORDER BY show_order ");
	...
	?>
        ...
        if($subcat_num_rows_new > 0)
		{
			...
			{
				$sql_item = mysql_query("SELECT * FROM restaurant_menu_item WHERE category_id = '".$row_cat_new_div['id']."' AND restaurant_id = '".$restaurant_id."' AND sub_category_id = '".$row_subcat_new['id']."' ORDER BY show_order ");
			?>
                ...
                <table class="blue-table restaurent_menu resturant_menu_section" width="100%">
                <tr>
                ...
                <?php
				$min_order_id_itm = mysql_fetch_array(mysql_query("SELECT MIN(show_order) as min_id FROM restaurant_menu_item WHERE category_id = '".$row_cat_new_div['id']."' AND restaurant_id = '".$restaurant_id."' AND sub_category_id = '".$row_subcat_new['id']."' "));
                $max_order_id_itm = mysql_fetch_array(mysql_query("SELECT MAX(show_order) as max_id FROM restaurant_menu_item WHERE category_id = '".$row_cat_new_div['id']."' AND restaurant_id = '".$restaurant_id."' AND sub_category_id = '".$row_subcat_new['id']."' ")); 
				$item_inc = 0;
				while($row_item = mysql_fetch_assoc($sql_item))
				{
					$sql_ins = mysql_query("SELECT * FROM restaurant_menu_special_instruction WHERE menu_id = '".$row_item['id']."' AND restaurant_id = '".$restaurant_id."'");
				?>
                <tr>
                <td>
					...
					<div class="blue_table_top_color"><input type="text" name="menu_item_name[]" id="menu_item_name" class="restaurant" value="<?php echo $row_item['menu_name']; ?>" ></div>
                    <div class="blue_table_bottom_color"><input type="checkbox" name="whats_good_<?php echo $row_item['id']."_".$item_inc; ?>" id="whats_good_<?php echo $row_item['id']."_".$item_inc; ?>" value="1" <?php if($row_item['whats_good'] == '1'){?> checked="checked" <?php } ?>> What's Good</div>
                </td>
                <td>
					<div class="blue_table_top_color"><input type="text" name="menu_item_price[]" id="menu_item_price" class="restaurant menu_item_rest" value="<?php echo $row_item['price']; ?>" ></div>
                    <div class="blue_table_bottom_color"><input type="checkbox" name="spicy_<?php echo $row_item['id']."_".$item_inc; ?>" id="spicy_<?php echo $row_item['id']."_".$item_inc; ?>" value="1" <?php if($row_item['spicy'] == '1'){?> checked="checked" <?php } ?>> Spicy</div>
                </td>
                <td>
					<div class="blue_table_top_color"><input type="text" name="menu_item_description[]" id="menu_item_description" class="restaurant menu_item_desc" value="<?php echo $row_item['description']; ?>" ></div>
                    <div class="blue_table_bottom_color"><input type="checkbox" name="veggie_<?php echo $row_item['id']."_".$item_inc; ?>" id="veggie_<?php echo $row_item['id']."_".$item_inc; ?>" value="1" <?php if($row_item['veggie'] == '1'){?> checked="checked" <?php } ?>> Veggie</div>
                </td>
                <td>
                	
					<div class="blue_table_top_color"><input type="file" name="menu_itemwise_picture<?php echo $row_item['id']; ?>" id="menu_item_picture" class="restaurant_browse" >
                    <?php if($row_item['menu_pic'] != '') { ?>
					<img src="uploaded_images/<?php echo $row_item['menu_pic']; ?>" class="restu_browse_pic">
					<?php } ?>
                    </div>
                    <div class="blue_table_bottom_color"><input type="checkbox" name="healthy_<?php echo $row_item['id']."_".$item_inc; ?>" id="healthy_<?php echo $row_item['id']."_".$item_inc; ?>" value="1" <?php if($row_item['healthy'] == '1'){?> checked="checked" <?php } ?>> Healthy</div>
                </td>
                <td>
                <div class="blue_table_top_color"><a class="button4 menu-btn" style="font-size:16px;" href="javascript:void(0);" onClick="open_spl_ins_div('<?php echo $row_item['id']; ?>');">Add Special Instruction</a></div>
                
                <div id="spl_ins_div<?php echo $row_item['id']; ?>" style="display:none;" class="white_content1 spl_ins spl_pop"  >
				...
				{
					$sql_ins_item = mysql_query("SELECT * FROM restaurant_menu_item_special_instruction WHERE menu_id = '".$row_item['id']."' AND restaurant_id = '".$restaurant_id."' AND special_ins_id = '".$row_ins['id']."' ORDER BY show_order");
				?>
                    ...
                    <a href="spl_admin_restaurant_menu.php?&del_ins=<?php echo $row_ins['id']; ?>&menu_id=<?php echo $row_ins['menu_id']; ?>" style="margin-left:10px;" onClick="return confirm('Are you sure to delete this Entire Instruction?');"><img src="images/Close_Box_Red.png"></a>
                    ...
                    <?php
						$min_order_id_ins = mysql_fetch_array(mysql_query("SELECT MIN(show_order) as min_id FROM restaurant_menu_item_special_instruction WHERE menu_id = '".$row_item['id']."' AND restaurant_id = '".$restaurant_id."' AND special_ins_id = '".$row_ins['id']."' "));
                		$max_order_id_ins = mysql_fetch_array(mysql_query("SELECT MAX(show_order) as max_id FROM restaurant_menu_item_special_instruction WHERE menu_id = '".$row_item['id']."' AND restaurant_id = '".$restaurant_id."' AND special_ins_id = '".$row_ins['id']."' "));
					$option_inc = 1;  
                    ...
                    <tr>
                     ...
                     <a class="pop_up_arrow" href="order_change_up_ins.php?special_ins_id=<?php echo $row_ins_item['special_ins_id']; ?>&menu_id=<?php echo $row_ins_item['menu_id']; ?>&res_id=<?php echo $restaurant_id; ?>&show_order=<?php echo $row_ins_item['show_order']; ?>&id=<?php echo $row_ins_item['id']; ?>"> <img align="absmiddle" title="Order Up" alt="Order Up" src="images/icon-arrow-up-b-16.png"></a>
                     <?php } ?>
                     <?php if($max_order_id_ins['max_id']!= $row_ins_item['show_order']){ ?>
                     <a class="pop_down_arrow" href="order_change_down_ins.php?special_ins_id=<?php echo $row_ins_item['special_ins_id']; ?>&menu_id=<?php echo $row_ins_item['menu_id']; ?>&res_id=<?php echo $restaurant_id; ?>&show_order=<?php echo $row_ins_item['show_order']; ?>&id=<?php echo $row_ins_item['id']; ?>"> <img align="absmiddle" title="Order Down" alt="Order Down" src="images/icon-arrow-down-b-16.png"></a>
                     <?php } ?>
                     </td>
                     <td>
                     <a href="spl_admin_restaurant_menu.php?del_ins_opt=<?php echo $row_ins_item['id']; ?>&del_ins_item=1" onClick="return confirm('Are you sure to delete this Instruction option?');"><img src="images/Close_Box_Red.png"></a>
                     </td>
                    </tr>
...
<input class="button4" type="submit" value="Save & Continue" name="edit_spl_ins"  style="margin-right: 6px; margin-top: 6px;"></div>
                ...
                <a href="order_change_up_res_item.php?cat_id=<?php echo $row_item['category_id']; ?>&subcat_id=<?php echo $row_item['sub_category_id']; ?>&res_id=<?php echo $restaurant_id; ?>&show_order=<?php echo $row_item['show_order']; ?>&id=<?php echo $row_item['id']; ?>&sh_div=<?php echo $l; ?>&sh_itm=<?php echo $z; ?>&pg_name=spl_res"> <img align="absmiddle" title="Order Up" alt="Order Up" src="images/icon-arrow-up-b-16.png"></a>
                <?php } ?>
                <?php if($max_order_id_itm['max_id']!= $row_item['show_order']){ ?>
                <a href="order_change_down_res_item.php?cat_id=<?php echo $row_item['category_id']; ?>&subcat_id=<?php echo $row_item['sub_category_id']; ?>&res_id=<?php echo $restaurant_id; ?>&show_order=<?php echo $row_item['show_order']; ?>&id=<?php echo $row_item['id']; ?>&sh_div=<?php echo $l; ?>&sh_itm=<?php echo $z; ?>&pg_name=spl_res"> <img align="absmiddle" title="Order Down" alt="Order Down" src="images/icon-arrow-down-b-16.png"></a>
                <?php } ?>
                </td>
                <td><a href="spl_admin_restaurant_menu.php?del_item_id=<?php echo $row_item['id']; ?>&del_item=1&menu_id=<?php echo $row_ins_item['menu_id']; ?>" onClick="return confirm('Are you sure to delete this Item?');"><img src="images/Close_Box_Red.png"></a></td>
                </tr>
                <?php
				$item_inc++;
				}
	...
	<div id="state_city_div" style="display:none;" class="assign_item white_content"  >
						...
						<div class="close close-new" onClick="close_state_city_div();"><a href = "javascript:void(0);"> </a> </div>
                           ...
                           <span> City : </span>
                            ...
                            <input type="hidden" name="hid_res_id" id="hid_res_id" value="<?php echo $restaurant_id;?>" />
...
$all_cat = mysql_query("SELECT * FROM restaurant_menu_category WHERE restaurant_id IN (".$restaurant_id.")");

?>
<div class="l-contnt up-contnt"> 
        <form name="cat_subcat" id="cat_subcat" action="#" method="post" class="form-horizontal">
        <input type="hidden" id="hid_res_id" value="<?php echo $restaurant_id;?>" />
        <input type="hidden" id="itm_hid_id" />
                                   ...
                                   <span> Menu : </span>
                                    ...
                                    <input type="hidden" name="hid_res_id_ins" id="hid_res_id_ins" value="<?php echo $restaurant_id;?>" />
...
$sql_sel_cat = mysql_fetch_array(mysql_query("SELECT id FROM restaurant_menu_category WHERE restaurant_id IN (".$_REQUEST['restaurant_edit_id'].") ORDER BY show_order LIMIT 0,1"));
?>
