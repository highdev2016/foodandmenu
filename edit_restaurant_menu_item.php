...
<?php if($_REQUEST['action'] == 'delete' && $_REQUEST['spl_id']!=''){
	...
	header("location:edit_restaurant_menu_item.php?menu_id=".$_REQUEST['menu_id']."&restaurant_edit_id=".$_REQUEST['restaurant_edit_id']."&del_success=1");
}
?>
...
<?php if($_REQUEST['action'] == 'delete_option' && $_REQUEST['option_id']!=''){
	...
	header("location:edit_restaurant_menu_item.php?menu_id=".$_REQUEST['menu_id']."&restaurant_edit_id=".$_REQUEST['restaurant_edit_id']."&del_option_success=1");
}
?>
...
//print_r($_POST['sub_category1']);
	...
	foreach($_POST['countdiv'] as $countDiv){
		...
		while($array_select_spl_ins = mysql_fetch_array($sql_select_spl_ins)){
			$sql_update_spl_ins = mysql_query("UPDATE restaurant_menu_special_instruction SET special_instruction = '".htmlspecialchars(stripslashes($_POST['special_instruction_1_'.$j]),ENT_QUOTES)."' WHERE id = '".$array_select_spl_ins['id']."'");
	...
	header("location:edit_restaurant_menu_item.php?menu_id=".$_REQUEST['menu_id']."&restaurant_edit_id=".$restaurant_id."&success=1");
}
?>
...
<textarea name="sub_category_description1" id="sub_category_description1" rows="3" cols="25" style="border:1px solid #B5ABC6; margin-top:10px;">
<?php echo $sql_category['sub_category_description'];?></textarea></div>
