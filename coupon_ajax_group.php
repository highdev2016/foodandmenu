	...
	if(!empty($sql)){
		if($sql['end_date'] < $curr_date){
			...
			if($sql['online_redeem'] == 0){
				...
				{
					$_SESSION['coupon_code'.$ses_rest_id."_".$_SESSION['group_order_details_id'.$ses_rest_id]] = $_REQUEST['coupon_code'];
					$pre_discounted_price = ($amount * $sql['discount'])/100;
				
					$discounted_price = ($amount - $pre_discounted_price);
					
					$_SESSION['coupon_discount'.$ses_rest_id."_".$_SESSION['group_order_details_id'.$ses_rest_id]] = number_format($pre_discounted_price,2);
				...
				else
				{
					...
					$discounted_price = ($amount - $sql['coupon_price']);
