...
class profileupdateinfo 
{
	function profileupdateval($log)
	{
		...
		if($num_row!=0)
		{
			$profile_edit="UPDATE `restaurant_customer` SET `firstname` = '".$_REQUEST['firstname']."',
...
`zip` = '".$_REQUEST['zip']."',
`home_apartment` = '".$_REQUEST['home_apartment']."',
`apt_name` = '".$_REQUEST['apt_name']."',
`apt_no` ='".$_REQUEST['apt_no']."',
`gate_code` = '".$_REQUEST['gate_code']."',
`gender` = '".$_REQUEST['gender']."' WHERE `id` ='".$customer_id."'";
...
class resinfo 
{
	function allResval($log)
	{
		...
		$foodandmenu['resall'] = array();
                ...
                $sql_search = mysql_query("SELECT t1.*,t2.* FROM restaurant_basic_info as t1 , restaurant_business_delivery_takeout_info as t2 , restaurant_services_dress_payment as t3 WHERE t1.id = t2.restaurant_id AND t1.id = t3.restaurant_id GROUP BY t3.restaurant_id LIMIT ".$frm." ,".$num_item);
...
class coupan_resinfo 
{
	function coupan_allResval($log)
	{
		...
		$foodandmenu['resall'] = array();
                ...
                $sql_search = mysql_query("SELECT t1.*,t2.* FROM restaurant_basic_info as t1 , restaurant_business_delivery_takeout_info as t2 , restaurant_services_dress_payment as t3 WHERE t1.id = t2.restaurant_id AND t1.id = t3.restaurant_id GROUP BY t3.restaurant_id ORDER BY t3.restaurant_id DESC LIMIT ".$frm." ,".$num_item);
