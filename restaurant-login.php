...
if(isset($_REQUEST['submit'])){
	...
	$restaurant_id = loginRestaurant($email, $password);
	if($restaurant_id > 0){
		if($_REQUEST['checkbox'] == 1){
		$expire=time()+60*60*24*30;
		setcookie("restaurant_id", $restaurant_id, $expire); }
		//$_SESSION['restaurant_id'] = $restaurant_id;
		//$_SESSION['restaurant_user_name'] = $restaurant_id;
		//$_SESSION['restaurant_id'] = $restaurant_id;
		if($_SESSION['restaurant_user_type']=="admin_restaurant")
		{
			header("location:view_all_restaurant_admin.php");
		}
		elseif($_SESSION['restaurant_user_type']=="admin_restaurant_add")
		{
			header("location:add_restaurant_admin.php");
		}
		else{
...
<h2>You must sign in to add your restaurant</h2>
<h1>Log in</h1>
</div>
