...
if($_REQUEST['add'] == 'add'){
	...
	//$sql_quan = mysql_fetch_array(mysql_query("SELECT price, quantity FROM restaurant_menuitem_cart WHERE session_id = '".$session_id."' AND restaurant_id = '".$_REQUEST['id']."'"));
...
#fade
	...
	.pop_item2{position: absolute;margin:100px 0px 0px 0px;background:#f5f5f5;box-shadow:0px 1px 5px 0px #aeaeae;width: 500px;-moz-border-radius:5px;-webkit-border-radius:5px;border-radius:5px; z-index:999999999999; width:600px;}
                ...
                <div class="food_content">
                    ...
                    <h2 class="popular_item"><a href="#top_div" onClick="popularitem()">More Business Info... </a></h2>
                        ...
                        <table width="100%" border="0" cellspacing="1" class="restro2_table">
                          ...
                          <?php 
						  $amount = $amount + ($array_items['price']*$array_items['quantity']) - $_SESSION['coupon_discount'];
						  $amt1 = $amt1 + ($array_items['price']*$array_items['quantity']);
						  } ?>
                          
                          <?php if($_SESSION['coupon_code']!=''){ ?>
                              <tr id="discount_div" style="display:block;">
                                <td height="20" colspan="3" align="center" style="padding-top:35px;">&nbsp;</td>
                                <td height="20" align="right" style="padding-top:20px;">Discount</td>
                                <td height="20" id="discount_td" align="left" style="padding-left:10px; padding-top:20px;"><?php echo "$ ".$_SESSION['coupon_discount']; ?></td>
                              </tr>
                          <?php }?>
                          ...
                          <?php if(isset($_SESSION['del_charge'])){
							  $div_sty = 'block';
						  }else{
							  $div_sty = 'none';
						  } ?>
                          ...
                          <tr>
                            ...
                            <td height="20" align="left" style="padding-left:10px;"><?php echo "$ ".$_SESSION['del_charge']; ?></td>
                          </tr>
                          <tr>
                            <td height="20" colspan="3" align="center">&nbsp;</td>
                            <td height="20" align="right">Total </td>
                            <td height="20" align="left" style="padding-left:10px;"><?php echo "$ ". ($_SESSION['del_charge'] + $tax1 + $amount); ?></td>
                          </tr>
						  <?php }else { ?>
                          <tr>
                            <td height="20" colspan="3" align="center">&nbsp;</td>
					...
					function check_coupon_code(amount,res_id)
					{
						...
						$j.ajax({
							url : 'http://foodandmenu.com/coupon_ajax.php',
							type : 'POST',
							...
							success : function(data, textStatus, jqXHR){
								...
								{
									$j("#error_msg").html(data);
									$j("#error_msg").slideDown(1000);
									$j("#discount_div").slideUp(1000);
                    ...
                    Coupon Code : 
                    <div style="width:100%; margin:0 0 15px 10px;">
                    <input type="text" name="coupon_code" id="coupon_code" class="profilefield_right" style="float:left; width:154px; margin-right:11px; padding:5px;">
                    
                    <input type="button" name="apply" class="check_order_button" id="apply" onClick="check_coupon_code('<?php echo $amt1; ?>','<?php echo $res_id; ?>');" value="Apply" style="text-decoration:none; margin-top:10px; padding:3px 22px;">
                    </div>
                    <div id="error_msg" style="color:#F00; margin:-5px 0 4px 12px; display:none;"></div>
					...
					}elseif($_REQUEST['type'] == 'del' && isset($_SESSION['del_charge'])){
						$class_del = 'checked';
					}
					?>
...
google.maps.event.addDomListener(window, 'load', initialize);
                    ...
                    <?php if($sql_restaurant_delivery_details['pickup'] == 1 || $sql_restaurant_delivery_details['delivery'] == 1 || isset($_SESSION['del_charge'])){ ?>
                    <div id="check_out_del" style="display:none;">
                    <?php if(isset($_SESSION['del_charge']) && !isset($_SESSION['customer_id'])){
						$link3 = 'login.php?type=del';
					}elseif(isset($_SESSION['del_charge']) && isset($_SESSION['customer_id'])){
