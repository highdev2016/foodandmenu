...
function distance($start,$destination) {
		...
		$data = file_get_contents("https://maps.googleapis.com/maps/api/distancematrix/json?origins=".$from."&destinations=".$to."&language=en-EN&key=AIzaSyCm4dswtZfJ7Jvbz_PFBp08P6FqrUtjDrA");
...
class resprofileinfo 
{
	function profileval($log)
	{
		...
		$foodandmenu['resall']['firstname'] =$row_user['firstname'];
		$foodandmenu['resall']['lastname'] =$row_user['lastname'];
		$foodandmenu['resall']['email'] =$row_user['email'];
		$foodandmenu['resall']['address'] =$row_user['address'];
		$foodandmenu['resall']['phone'] =$row_user['phone'];
		$foodandmenu['resall']['image'] =$row_user['image'];
		$foodandmenu['resall']['city'] =$row_user['city'];
		$foodandmenu['resall']['state'] =$row_user['state'];
		$foodandmenu['resall']['zip'] =$row_user['zip'];
		$foodandmenu['resall']['home_apartment'] =$row_user['home_apartment'];
...
class resinfo 
{
	function allResval($log)
	{
		...
		$foodandmenu['resall'] = array();
                ...
                $sql_search = mysql_query("SELECT t1.*,t2.* FROM restaurant_basic_info as t1 , restaurant_business_delivery_takeout_info as t2 , restaurant_services_dress_payment as t3 WHERE t1.id = t2.restaurant_id AND t1.id = t3.restaurant_id LIMIT 0 , 200");
                ...
                while ($res_array = mysql_fetch_array($sql_search)) {
                    ...
                    if ($distance <= $search_distance['search_mile']) {
						...
						$sql_del_crge = mysql_query("SELECT * FROM restaurant_delivery_charge WHERE restaurant_id = '" . $res_array['id'] . "' ORDER BY delivery_range");
						$del_fee = 0.00;
...
$total_review=mysql_num_rows(mysql_query("SELECT * FROM restaurant_reviews where restaurant_id='".$res_array['restaurant_id']."' AND status=1"));
		...
		//$foodandmenu['resall']['review_this_restaurant'] =$total_review;
						...
						'restaurant_name' => $res_array['restaurant_name'],
						...
						'restaurant_image' => $res_array['restaurant_image'],
...
if($num_rows == 0){
	//echo "dd";
	$sql_insert_customer = mysql_query("INSERT INTO restaurant_customer SET firstname = '".$first_name."' , lastname = '".$last_name."', email = '".$email."' , gender = '".$gender."' , oauth_uid = '".$id."' , oauth_provider = '".$provider."' , 	registration_time = NOW() , city = '".$city."' , status = '1' , last_logged_in = '".date('Y-m-d H:i:s')."'");
	...
	$sql_update_customer = mysql_query("UPDATE restaurant_customer SET  gender = '".$gender."' , status = '1' , last_logged_in = '".date('Y-m-d H:i:s')."' , oauth_provider = 'Google Plus' WHERE oauth_uid = '".$id."'");	
	$customer_id=$row_select['id'];
}

$foodandmenu['resall']['customer_id'] =$customer_id;
$foodandmenu['resall']['firstname']=$row_select["firstname"];
$foodandmenu['resall']['lastname']=$row_select["lastname"];
$foodandmenu['resall']['email']=$row_select["email"];
$foodandmenu['resall']['address']=$row_select["address"];
$foodandmenu['resall']['phone']=$row_select["phone"];
$foodandmenu['resall']['city']=$row_select["city"];
$foodandmenu['resall']['state']=$row_select["state"];
$foodandmenu['resall']['zip']=$row_select["zip"];
...
class resdetail 
{
	function restaurant_detail($log){
		...
		while($array_menu_category = mysql_fetch_array($sql_resataurant_menu_category)){
			...
			$foodandmenu['menu_item'] = array();
				while($array_restaurant_menu = mysql_fetch_array($sql_restauarnt_menu)){
					$sql_special_ins = mysql_query("SELECT * FROM restaurant_menu_special_instruction WHERE menu_id = '".$array_restaurant_menu['id']."' ");
					$foodandmenu['spl_ins'] = array();
					while($array_special_ins = mysql_fetch_array($sql_special_ins)){
						$sql_special_ins_option = mysql_query("SELECT * FROM restaurant_menu_item_special_instruction WHERE special_ins_id = '".$array_special_ins['id']."' ");
			...
			array_push($foodandmenu['resall'], array(
						'category_name' => $array_menu_category['category_name']
						,
						$foodandmenu['subcat']
						));
		...
		$restaurant_id = $_REQUEST['id'];
		 ...
		 while($array_coupon = mysql_fetch_array($sql_coupons)){
			 array_push($foodandmenu['restaurant_coupon'], array(
						'coupon_name' => $array_coupon['coupon_name'],
						...
						'coupon_description' => $array_coupon['coupon_description'],
...
class rescouponinfo
{
	...
	if($res_coupon['coupon_name']!="")
	   {
		$foodandmenu['coupon_status']="valid";
	   }
	   else
	      {
			$foodandmenu['coupon_status']="Invalid";  
		  }
	return $foodandmenu['coupon_status'];
...
class reswrite_review 
{
	function restaurant_write_review($log){
			...
			if($review_image!=''){
				...
				mysql_query("INSERT INTO restaurant_photo SET image_name = '".$review_image."' , restaurant_id = '".$restaurant_id."'");
			}
...
class res_rating_reviews 
{
	function restaurant_rating_reviews($log){
		...
		$sql_customer = mysql_query("SELECT DISTINCT(customer_id) FROM restaurant_reviews WHERE restaurant_id = '".$restaurant_id."' ");
		$foodandmenu['customer_reviews'] = array();
		while($array_customer = mysql_fetch_array($sql_customer)){
			$sql_customer_details = mysql_fetch_array(mysql_query("SELECT * FROM restaurant_customer WHERE id = '".$array_customer['customer_id']."' "));
			$sql_select_latest_review = mysql_fetch_array(mysql_query("SELECT * FROM restaurant_reviews WHERE restaurant_id = '".$restaurant_id."' AND customer_id = '".$array_customer['customer_id']."' ORDER BY post_date DESC "));
			$sql_review_images = mysql_fetch_array(mysql_query("SELECT * FROM restaurant_review_images WHERE review_id = '".$sql_select_latest_review['id']."' ORDER BY RAND() LIMIT 0,1 "));
			$sql_follow = mysql_num_rows(mysql_query("SELECT * FROM user_follow WHERE follower_id = '".$user_id."' AND following_id = '".$array_customer['customer_id']."' "));
			...
			array_push($foodandmenu['customer_reviews'], array(
								'customer_id' => $array_customer['customer_id'],
								'customer_name' => $sql_customer_details['firstname']." ".$sql_customer_details['lastname'],
								'customer_image' => $sql_customer_details['image'],
								'post_date' => $sql_select_latest_review['post_date'],
								'review' => $sql_select_latest_review['customer_review'],
								'rating' => getSingleReviewRating($restaurant_id,$sql_select_latest_review['id']),
								'review_images' => $sql_review_images['image'],
								'follow_status' => $follow
								));
		}
			
		return $foodandmenu['customer_reviews'];		
	}
}
...
class res_order_history 
{
	function restaurant_order_history($log){
		...
		$foodandmenu['restaurant_order_history'] = array();
		
		while($order_history = mysql_fetch_array($sql_order))
		{
			$restaurant_name = mysql_fetch_array(mysql_query("SELECT restaurant_name FROM restaurant_basic_info WHERE id = '".$order_history['restaurant_id']."'"));
			$sql_order_details = mysql_query("SELECT * FROM restaurant_food_order_details WHERE order_id = '".$order_history['order_id']."'");
			
			$foodandmenu['restaurant_order_details'] = array();
			
			while($row_order_details = mysql_fetch_array($sql_order_details))
			{
				array_push($foodandmenu['restaurant_order_details'],array(
					'menu_id' => $row_order_details['menu_id'],
					'menu_name' => $row_order_details['menu_name'],
					...
					'special_instructions' => $row_order_details['menu_id'],
					'additional_instruction' => $row_order_details['menu_id']
				));
			}
			
			array_push($foodandmenu['restaurant_order_history'],array(
							'order_id' => $order_history['order_id'],
							'restaurant_id' => $order_history['restaurant_id'],
							'reastaurant_name' => $restaurant_name,
							...
							'status' => $order_history['total_price'],
							'time' => $order_history['status'],
