...
<?php $sql_basic_info_city=mysql_query("SELECT DISTINCT(restaurant_city) FROM restaurant_basic_info where status=1");
$sep="";
$all_county="";
while($res_basc_info_city=mysql_fetch_array($sql_basic_info_city))
{
	$all_city.=$sep.'"'.$res_basc_info_city['restaurant_city'].'"';
	$sep=",";
}
...
{
	$all_state.=$sep.'"'.$res_basc_info_state['restaurant_state'].'"';
	$sep=",";
}
...
{
	$all_zipcode.=$sep.'"'.$res_basc_info_zipcode['restaurant_zipcode'].'"';
	$sep=",";
}

 $all_total=$all_city.$all_state.$all_zipcode;
 ?>

<script type="text/javascript">
var suggestions = new Array(<?php echo $all_total?>);
...
function setVisible(visi)
{
  ...
  var t = document.getElementsByName("city")[0];
...
function lookAt()
{
   var ins = document.getElementsByName("city")[0].value;
   if (oldins == ins)
      return;
   else if (posi > -1);
   else if (ins.length > 0)
   {
     ...
     if (words.length > 0)
     {
        ...
        for (var i=0;i < words.length; ++i)
             addWord (words[i]);
        setVisible("visible");
        input = document.getElementsByName("city")[0].value;
     }
     else
     {
        setVisible("hidden");
...
function keyHandler(event)
{
  ...
  {
     var textfield = document.getElementsByName("city")[0];
     if (key == 40)//key down
     { 
    ...
    var mouseHandler=function()
    {
     ...
     {
        document.getElementsByName("city")[0].value = this.firstChild.nodeValue;
        setVisible("hidden");
        posi = -1;
