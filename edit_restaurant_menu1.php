...
if($_REQUEST['del_item'] == '1')
{
	...
	header("location:edit_restaurant_menu.php?restaurant_edit_id=".$_REQUEST['restaurant_edit_id']."&itemdel=1");
}
...
if($_REQUEST['submit'] == 'Save & Continue'){
	...
	header("location:edit_restaurant_menu.php?restaurant_edit_id=".$restaurant_id."&item_add=1");
...
if($_REQUEST['submit_item'] == 'Save & Continue')
{
	...
	if($update)
	{
		header("location:edit_restaurant_menu.php?restaurant_edit_id=".$_REQUEST['restaurant_edit_id']."&item_edit=1&sct=".$subcat."&top=".$top_div."&accor=".$accor_div."");
	}
}
...
function add_spl_ins_option(option_id,spl_ins_id,menu_id,id){
	...
	$.ajax({
		...
		success : function( data, textStatus, jqXHR){
			...
			document.getElementById('add_ins_option_id_'+id+'_'+menu_id+'_'+spl_ins_id).value=parseInt(option_id)+1;
		},
		/*complete : function( jqXHR, textStatus){
			alert(3);
		},*/
...
<div id="loader_div_ins" class="center-loder" style="display:none;" style="margin-left: 80px;margin-top: -2px;"><img src="images/ajax-loader-review.gif" /></div>
