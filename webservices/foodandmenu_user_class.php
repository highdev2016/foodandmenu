...
class rescouponinfo
{
	...
	{
		$today = date("Y-m-d");
		$total_price= trim($_REQUEST['totprice']);
     	$res_coupon = mysql_fetch_array(mysql_query("SELECT `coupon_name`,`coupon_price`,`discount`  FROM `restaurant_coupon` WHERE `coupon_code` = '".$_REQUEST['coupon_code']."'AND  CURDATE() < `end_date`"));
	if($res_coupon['coupon_name']!="")
	   { 
			$coupon_use=mysql_query("SELECT count(*) as total_row from restaurant_menu_order WHERE `coupon_code` = '".$_REQUEST['coupon_code']."'AND `customer_id`= '".$_REQUEST['customer_id']."'");
			$data=mysql_fetch_assoc($coupon_use);
			if($data['total_row'] > 0)
			{
				$foodandmenu['resall']['coupon_status']="Coupan has already been used";
			}
			else
			{
				$foodandmenu['resall']['coupon_status']="valid";
				if($res_coupon['coupon_price'] > 0)
				{
				$total_price = $total_price - $res_coupon['coupon_price'];
				}
				else
				{
				$total_price = $total_price - (($res_coupon['discount']*$total_price)/100);
				}
				$foodandmenu['resall']['total_price']=$total_price;
			}
...
class ressignup 
{
	function restaurant_ressignup($log){
		...
		$check_data = mysql_query("SELECT `id` FROM restaurant_customer where `email`='".$email."'");
		$norow = mysql_num_rows($check_data);
		if($norow > 0)
		{
