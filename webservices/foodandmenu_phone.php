...
$resval = new resinfo();
$rating_resval = new rating_resinfo();
$city_state = new city_state_resinfo();
$hot_resval = new hot_resinfo();
$coupan_resval = new coupan_resinfo();

$changepassword = new changepassword();
$profileImageUpload = new profileImageUpload();
$restaurant_coupon_val = new restaurant_couponinfo();
$restaurant_hot_val = new restaurant_hot_info();
$restaurant_reviews_val = new restaurant_reviews_info();
$rescouponval = new rescouponinfo();
$resprofileval = new resprofileinfo();
$resreviewval = new resreviewinfo();
$resrhourval = new rhourinfo();
$resthankval = new thankinfo();

$resputlike = new putlike();
$resputdlike = new putdlike();

$resginfo = new ginfo();
$reslastdeliveryaddress=new lastdeliveryaddress();

$resprofileupdate=new profileupdateinfo();
$resdetail = new resdetail1();
...
$res_lastorder_status = new res_lastorder_status();
...
{	
	...
	case "rating":
            $all_res=$rating_resval->rating_allResval($_REQUEST);
	break;
	
	case "changepassword":
            $all_res=$changepassword->do_changepassword($_REQUEST);
	break;
	
	case "hot_deals":
            $all_res=$hot_resval->hot_allResval($_REQUEST);
	break;
	
	case "profileImageUpload":
            $all_res=$profileImageUpload->profileimageUpload($_REQUEST);
	break;
			
	case "coupan":
            $all_res=$coupan_resval->coupan_allResval($_REQUEST);
	break;
	
	
	case "restaurant_coupon":
            $all_res=$restaurant_coupon_val->restaurant_coupon($_REQUEST);
	break;
	
	
	
	case "restaurant_hot":
            $all_res=$restaurant_hot_val->restaurant_hot($_REQUEST);
	break;
	
	
	
	case "restaurant_reviews_top":
            $all_res=$restaurant_reviews_val->restaurant_reviews_top($_REQUEST);
	break;
	...
	case "profile_update":
            $all_res=$resprofileupdate->profileupdateval($_REQUEST);
	break;
	...
	break;
	case "rhour":
	     $all_res=$resrhourval->rhourval($_REQUEST);
	break;
		
		case "thankyou":
	     $all_res=$resthankval->thankval($_REQUEST);
	break;
	
	case "putlike":
	     $all_res=$resputlike->putlikeval($_REQUEST);
	break;	
		
	case "putdlike":
	     $all_res=$resputdlike->putdlikeval($_REQUEST);
	break;		
		
	
	case "gfplus":
	     $all_res=$resginfo->saveg($_REQUEST);
	break;
	...
	case "restaurant_detail":
            $all_res=$resdetail->restaurant_detail1($_REQUEST);
	break;
	case "restaurant_coupon":
    ...
    break;
	case "lastorder_status":
            $all_res=$res_lastorder_status->restaurant_lastorder($_REQUEST);
    break;
	case "city_state":
            $all_res=$city_state->restaurant_city_state($_REQUEST);
    break;
}
