...
if(count($_POST)>0 && $_REQUEST['submit']=="Save & Continue")
{
	...
	foreach($googlemap->results as $res){
		...
		$formattedaddress = $res->formatted_address;
	?>
	<?php echo "Latitude: ". $latlng->lat ."<br />". "Longitude:". $latlng->lng; exit;
	mysql_query("UPDATE restaurant_basic_info SET latitude = '".$latlng->lat."', longitude = '".$latlng->lng."' WHERE id = '".$_SESSION['rest_id']."'");
	?>
	<?php
	}
	}
...
{
?>
<input name="name2" id="name2" type="text" class="restaurant" value="<?php echo $user_info['user_nicename']; ?>" readonly />
<?php
}
else{
?>
