...
if(count($_POST)>0 && $_REQUEST['submit']=="Save & Continue")
{
	if($_FILES['daily_picture']!="")
		...
		mysql_query("update restaurant_deals set deals_status='".$_REQUEST['daily_deals']."',daily_name='".$_REQUEST['daily_name']."',daily_price='".$_REQUEST['daily_price']."',daily_description='".$_REQUEST['daily_description']."',expiry_date='".change_dateformat($_REQUEST['expiry_date'])."',daily_picture='".$video_image."',special_rules='".$_REQUEST['special_rules']."',disclaimer='".$_REQUEST['disclaimer']."' where id='".$_REQUEST['deals_hid']."'");
		
		}
		else{
		
			mysql_query("update restaurant_deals set deals_status='".$_REQUEST['daily_deals']."',daily_name='".$_REQUEST['daily_name']."',daily_price='".$_REQUEST['daily_price']."',daily_description='".$_REQUEST['daily_description']."',expiry_date='".change_dateformat($_REQUEST['expiry_date'])."',special_rules='".$_REQUEST['special_rules']."',disclaimer='".$_REQUEST['disclaimer']."'  where id='".$_REQUEST['deals_hid']."'");
			
		}
		
		header("location:admin_special_offer.php?success=4");
...
<input name="disclaimer" id="disclaimer" type="text" class="restaurant_browse" value="<?php echo $select_deals['disclaimer'];?>" />
