...
if($_REQUEST['submit_category'] == 'Create'){
	...
	if(mysql_num_rows($sql_sel_category) == '0'){
		$sql_category_show_order = mysql_fetch_array(mysql_query("SELECT * FROM restaurant_menu_category WHERE restaurant_id = '".$_REQUEST['restaurant_edit_id']."' ORDER BY show_order DESC LIMIT 0,1"));
		
		$sql_insert_category = mysql_query("INSERT INTO restaurant_menu_category SET restaurant_id = '".$_REQUEST['restaurant_edit_id']."', category_name = '".$_REQUEST['restaurant_category']."' , show_order = '".($sql_category_show_order['show_order']+1)."'");
...
{
	$check_dup = mysql_num_rows(mysql_query("SELECT id FROM restaurant_menu_category WHERE category_name = '".$_REQUEST['restaurant_category']."' AND id != '".$_REQUEST['cat_edit_id']."' AND restaurant_edit_id='".$_REQUEST['restaurant_edit_id']."'"));
	if($check_dup == 0)
	{
		...
		header("location:edit_restaurant_menu.php?restaurant_edit_id=".$_REQUEST['restaurant_edit_id']."&catedit_success=1&popup=1&edt=".$_REQUEST['pop_tab_id']."");
	}
	else
	{
		header("location:edit_restaurant_menu.php?restaurant_edit_id=".$_REQUEST['restaurant_edit_id']."&catedit_error=1&popup=1&edt=".$_REQUEST['pop_tab_id']."");
	}
...
if($_REQUEST['submit_cat_time'] == "Submit")
{
	...
	if($update)
	{
		header("location:edit_restaurant_menu.php?restaurant_edit_id=".$_REQUEST['restaurant_edit_id']."&cattimesuccess=1&popup=1");
	}
...
if($_REQUEST['submit_sub_cat_time'] == "Submit")
{
	...
	if($update)
	{
		header("location:edit_restaurant_menu.php?restaurant_edit_id=".$_REQUEST['restaurant_edit_id']."&subcattimesuccess=1&popup=1");
	}
...
<div id="nw_pop" class="white_content xslg">
   ...
   <?php if($_REQUEST['catedit_success'] == '1'){ ?> 
   <p style="color:#090; text-align:center;">Category Edited Successfully.</p>
   <?php } ?>
   
   <?php if($_REQUEST['catedit_error'] == '1'){ ?> 
   <p style="color:#F00; text-align:center;">Category Already Exsists.</p>
   <?php } ?>
   ...
   <?php } ?>
	...
	<?php
	$sql_cat = mysql_query("SELECT * FROM restaurant_menu_category WHERE restaurant_id IN (".$_REQUEST['restaurant_edit_id'].") ORDER BY show_order");
	$sql_cat_single = mysql_query("SELECT * FROM restaurant_menu_category WHERE restaurant_id = ".$_REQUEST['restaurant_edit_id']." ORDER BY show_order");
	?>
    <div style="height:300px; overflow:auto;">
    
    <div class="full-width pop-table" id="res_menu_cat_<?php echo $_REQUEST['restaurant_edit_id']; ?>">
        <table cellpadding="0" cellspacing="0" class="blue-table"> 
		   ...
		   $sql_subcat2 = mysql_query("SELECT * FROM restaurant_menu_subcategory WHERE category_id = '".$row_cat_single['id']."' AND restaurant_id = '".$_REQUEST['restaurant_edit_id']."' ORDER BY show_order ");
			
			$min_order_id = mysql_fetch_array(mysql_query("SELECT MIN(show_order) as min_id FROM restaurant_menu_category WHERE restaurant_id = '".$_REQUEST['restaurant_edit_id']."' "));
                $max_order_id = mysql_fetch_array(mysql_query("SELECT MAX(show_order) as max_id FROM restaurant_menu_category WHERE  restaurant_id = '".$_REQUEST['restaurant_edit_id']."' "));
            ...
            <tr>
                ...
                <a href="order_change_up_res_cat.php?res_id=<?php echo $_REQUEST['restaurant_edit_id']; ?>&show_order=<?php echo $row_cat_single['show_order']; ?>&id=<?php echo $row_cat_single['id']; ?>&pg_name=edit_res"> <img src="images/icon-arrow-up-b-16.png" alt="Order Up" title="Order Up" align="absmiddle" /></a>
            <?php } ?>
            <?php if($max_order_id['max_id']!= $row_cat_single['show_order']){ ?>
                <a href="order_change_down_res_cat.php?res_id=<?php echo $_REQUEST['restaurant_edit_id']; ?>&show_order=<?php echo $row_cat_single['show_order']; ?>&id=<?php echo $row_cat_single['id']; ?>&pg_name=edit_res"> <img src="images/icon-arrow-down-b-16.png" alt="Order Down" title="Order Down" align="absmiddle" /></a>
            <?php } ?>
                            </td>
                <td align="center">
                <a href="edit_restaurant_menu.php?restaurant_edit_id=<?php echo $_REQUEST['restaurant_edit_id']; ?>&cat_edit_id=<?php echo $row_cat_single['id'];?>&edit_cat=1&popup=1&cat_edt=<?php echo $inc_cat; ?>"> <img src="images/pen_edit_write_-16.png" alt="Edit" title="Edit" align="absmiddle"></a>
				...
				{
				?>
                	<a href="edit_restaurant_menu.php?restaurant_edit_id=<?php echo $_REQUEST['restaurant_edit_id']; ?>&del_cat_id=<?php echo $row_cat_single['id']; ?>&del_cat=1" onClick="return confirm('Are you sure to Delete this Category?');"> <img src="images/DeleteRed.png" alt="Delete" title="Delete" align="absmiddle"></a>
                <?php
				}
				else
				{
	...
	<div class="restaurant_nav" style="padding:5px 0 0 0;">
		<ul>
		<?php
		$j = 1;
		...
		?>
		    <li class="li_cls"><?php /*?><a href="edit_restaurant_menu.php?restaurant_edit_id=<?php echo $_REQUEST['restaurant_edit_id']; ?>&del_cat_id=<?php echo $row_cat['id']; ?>&del_cat=1" onClick="return confirm('Are you sure to Delete this Category?');"><img class="del_img" src="<?php echo $image_path; ?>" width="10" style="display:none" alt="Delete" title="<?php echo $title; ?>" align="absmiddle"></a><?php */?><a href="javascript:void(0);" id="cat_anc<?php echo $j; ?>" onClick="open_cat_div('<?php echo $j; ?>');"><?php echo $row_cat['category_name']; ?></a></li>
    ...
    <?php
	$sql_cat1 = mysql_query("SELECT * FROM restaurant_menu_category WHERE restaurant_id IN (".$_REQUEST['restaurant_edit_id'].") ORDER BY show_order ");
	$jj = 1;
...
<?php 
$sql_cat_single1 = mysql_query("SELECT * FROM restaurant_menu_category WHERE restaurant_id = ".$_REQUEST['restaurant_edit_id']." ORDER BY show_order");
...
<?php
	$sql_cat_new = mysql_query("SELECT * FROM restaurant_menu_category WHERE restaurant_id IN (".$_REQUEST['restaurant_edit_id'].") ORDER BY show_order"); 
        ...
        if($subcat_num_rows_new > 0)
		{
			...
			?>
                ...
                <td>                	
                    ...
                    <div class="blue_table_bottom_color"><input type="checkbox" name="healthy_<?php echo $row_item['id']."_".$item_inc; ?>" id="healthy_<?php echo $row_item['id']."_".$item_inc; ?>" value="1" <?php if($row_item['healthy'] == '1'){?> checked="checked" <?php } ?>> Healthy
                                                            
                    </div>
                </td>
                <td><div class="blue_table_top_color"><a class="button4 menu-btn" style="font-size:16px;" href="javascript:void(0);" onClick="open_spl_ins_div('<?php echo $row_item['id']; ?>');">Add Special Instruction</a>                
                </div>
