...
if(count($_POST)>0 && $_REQUEST['submit']=="Save & Continue")
{
	if($_FILES['daily_picture']!="")
		...
		mysql_query("update restaurant_coupon set coupon_status='".$_REQUEST['coupon_deal']."',coupon_name='".$_REQUEST['coupon_name']."', coupon_price = '".$_REQUEST['coupon_price']."', discount =  '".$_REQUEST['discount_percent']."', start_date = '".change_dateformat($_REQUEST['start_date'])."' , end_date = '".change_dateformat($_REQUEST['end_date'])."' ,coupon_description='".$_REQUEST['coupon_description']."', coupon_print = '".$_REQUEST['coupon_print']."', apply_coupon = '".$_REQUEST['apply_coupon']."', reappear_date = '".change_dateformat($_REQUEST['reappear_date'])."', coupon_image='".$video_image."' , minimum_order_amount = '".$_REQUEST['minimum_order_amount']."' , show_coupon = '".$_REQUEST['show_coupon']."' , show_coupon_code = '".$_REQUEST['show_coupon_code']."' , online_redeem = '".$_REQUEST['online_status']."'  where id='".$_REQUEST['deals_hid']."'");
		
		}
		else{
			
			mysql_query("update restaurant_coupon set coupon_status='".$_REQUEST['coupon_deal']."',coupon_name='".$_REQUEST['coupon_name']."', coupon_price = '".$_REQUEST['coupon_price']."', discount = '".$_REQUEST['discount_percent']."', start_date = '".change_dateformat($_REQUEST['start_date'])."' , end_date = '".change_dateformat($_REQUEST['end_date'])."' ,coupon_description='".$_REQUEST['coupon_description']."' , coupon_print = '".$_REQUEST['coupon_print']."', apply_coupon = '".$_REQUEST['apply_coupon']."', reappear_date = '".change_dateformat($_REQUEST['reappear_date'])."', minimum_order_amount = '".$_REQUEST['minimum_order_amount']."' , show_coupon = '".$_REQUEST['show_coupon']."' , show_coupon_code = '".$_REQUEST['show_coupon_code']."' , online_redeem = '".$_REQUEST['online_status']."' where id='".$_REQUEST['deals_hid']."'");
			
		}
		
		header("location:edit_special_offer.php?restaurant_edit_id=".$_REQUEST['restaurant_edit_id']."&success=1");
...
$j( document ).ready(function() {
	$j(window).on('load',function() {
	  $j('#reappear_date').datepicker({changeMonth: true, changeYear: true, dateFormat:"mm-dd-yy",});
	});	  
});
...
<div class="restaurant_form_field" style="">
...
<input name="coupon_print" id="coupon_print" type="text" class="restaurant" value="<?php echo $select_deals['coupon_print'];?>" />

<div class="clear"></div>

<p>No. of times Apply Coupon* :</p>

<input name="apply_coupon" id="apply_coupon" type="text" class="restaurant" value="<?php echo $select_deals['apply_coupon'];?>" />

<div class="clear"></div>

<p>Reappear Date :</p>

<input name="reappear_date" id="reappear_date" type="text" class="restaurant" value="<?php if($select_deals['reappear_date'] != "0000-00-00") echo date("m-d-Y", strtotime($select_deals['reappear_date']));?>" />
...
<a href="restaurant_live_orders.php"><input class="button4 edit_submit" style="margin-left:10px;"  value="Back" name="submit1"></a>
