...
function main()
{
	...
	if($image!="")
	{
			mysql_query("update restaurant_banner set text='".mysql_real_escape_string($_REQUEST['text'])."',link='".mysql_real_escape_string($_REQUEST['link'])."',image='".$image."',status='".mysql_real_escape_string($_REQUEST['status'])."' where id='".$_REQUEST['id']."'");	
	}
	else{
		mysql_query("update restaurant_banner set text='".mysql_real_escape_string($_REQUEST['text'])."',link='".mysql_real_escape_string($_REQUEST['link'])."',status='".mysql_real_escape_string($_REQUEST['status'])."' where id='".$_REQUEST['id']."'");
	}	
	header("location:manage_banner.php?success=3");
	}
	
	$sql_banner=mysql_fetch_array(mysql_query("SELECT * FROM restaurant_banner WHERE id='".$_REQUEST['id']."'"));
?>
...
<form name="frmchangpass" action="" method="post" onsubmit="return validate()" enctype="multipart/form-data">
  ...
  <tr>
   ...
   <tr>
    ...
    <td class="normaltext"><input name="text" id="text" type="text" class="login_ipboxin"  value="<?php echo $sql_banner['text'];?>"></td>
  </tr>
  <tr>
