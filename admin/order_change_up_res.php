<?php  ob_start();
	...
	$sql_team_order=sprintf("SELECT show_order,id FROM restaurant_basic_info WHERE show_order<'%s' order by show_order desc limit 0,1",$order);
	...
	$Current_team_id=mysql_fetch_array(mysql_query(sprintf("SELECT id FROM restaurant_basic_info WHERE show_order='%s'",  $order)));
	if($row_team_order['show_order']!="")
	{
		...
		$sql_update_team_order=mysql_query(sprintf("UPDATE restaurant_basic_info set show_order='%s' where id='%s'",
											$next_order,
											$Current_order_team_id));
		$sql_update_team_order_next=mysql_query(sprintf("UPDATE restaurant_basic_info set show_order='%s' where id='%s'",
											$order,
											$next_order_team_id));	
		if($_REQUEST['page']!=''){							
