...
if($_REQUEST['submit'] == 'Send'){
	...
	/*if(isset($_POST['send_to']) && !empty($_POST['send_to'])){*/
		...
		$url="<a href='http://www.foodandmenu.com/newsletters_unsubscribe.php?action=view&id=".$sql['id']."' target='_blank'>
	http://www.foodandmenu.com/newsletters_unsubscribe.php?action=view&id=".$sql['id']."</a>";
...
<input type="hidden" name="subs_city" id="subs_city" value="" />
  ...
  <?php $sql_select = mysql_query("SELECT DISTINCT(subscriber_city) FROM restaurant_subscriber WHERE subscriber_city!='' AND status = 1");
  if(mysql_num_rows($sql_select)>0){?>
  <tr>
    <td class="text1" align="right" valign="top">City :</td>
    <td class="text1" style="text-align:left;">
    <?php while($array_select = mysql_fetch_array($sql_select)){ ?>
  ...
  <tr>
    ...
    <div style="border: 1px solid #000000; height: 200px; overflow: auto; padding-left: 5px; width: 400px;"><div id="send_to_div">
    <?php
	$sql_send = mysql_query("SELECT id,email FROM restaurant_subscriber WHERE status = 1");
	$count = 1;
