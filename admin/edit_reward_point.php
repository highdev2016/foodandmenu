...
function main()
{
	$array_reward = mysql_fetch_array(mysql_query("SELECT * FROM restaurant_multiple_reward WHERE id = '".$_REQUEST['id']."'"));
	...
	if(count($_POST)>0 && $_REQUEST['submit']=="Update")
	{
		...
		mysql_query("update restaurant_multiple_reward set name='".mysql_real_escape_string($_REQUEST['name'])."',description='".mysql_real_escape_string($_REQUEST['description'])."',reward_type='".$type."', point = '".$_REQUEST['reward_point']."' , redeemable_point = '".$_REQUEST['redeemable_point']."', start_date = '".change_dateformat_reverse($_REQUEST['start_date'])."', end_date = '".change_dateformat_reverse($_REQUEST['end_date'])."', status='".$_REQUEST['status']."' , promote = '".$_REQUEST['promote']."' , post_date = NOW(),price='".$_REQUEST['amount']."' where id = '".$_REQUEST['id']."'");
		...
		header("location:manage_rewards.php?success=3");
...
<h2 style="background:#FA8730; color:#fff; padding:8px;">Edit Reward Point</h2>
<form name="frmchangpass" action="" method="post" onsubmit="return validate()" enctype="multipart/form-data">
            	<table width="90%" border="0" cellspacing="2" cellpadding="2"  style="padding:3px;" height="160" align="center" >
  <tr>
    <td colspan="2" class="text2"></td>
  </tr>
  <tr>
    <td class="text1" align="right">Name <font color="#FF0000">*</font>:</td>
    <td class="normaltext"><input name="name" id="name" type="text" class="login_ipboxin"  value="<?php echo $array_reward['name']; ?>"></td>
  </tr>
  <tr>
    <td class="text1" align="right">Description :</td>
    <td class="normaltext"><textarea id="description" cols="25" rows="5" name="description" style="width: 282px; height: 92px;"><?php echo stripslashes(htmlspecialchars_decode($array_reward['description'])); ?></textarea>
    <script type="text/javascript">
   ...
   <tr>
	...
	$reward_type_db = explode(",",$array_reward['reward_type']); 
	?>
    <input type="checkbox" name="reward_type[]" value="<?php echo $array_sel['id']; ?>"<?php if(in_array($array_sel['id'],$reward_type_db)) { ?> checked <?php } ?>  /><span class="text1"> <?php echo $array_sel['name']; ?> </span><br />
    <?php } ?> </td>
  </tr>
  <tr>
    <td class="text1" align="right">Reward Point <font color="#FF0000">*</font>:</td>
    <td class="normaltext"><input name="reward_point" id="reward_point" type="text" class="login_ipboxin"  value="<?php echo $array_reward['point']; ?>" onkeypress="return goodchars(event,'1234567890.');"></td>
  </tr>
  <tr>
    <td class="text1" align="right">Redeemable Point <font color="#FF0000">*</font>:</td>
    <td class="normaltext"><input name="redeemable_point" id="redeemable_point" type="text" class="login_ipboxin"  value="<?php echo $array_reward['redeemable_point']; ?>" onkeypress="return goodchars(event,'1234567890.');"></td>
  </tr>
  <tr>
    <td class="text1" align="right">Start Date <font color="#FF0000">*</font>:</td>
    <td class="normaltext"><input name="start_date" id="start_date" type="text" class="login_ipboxin" readonly  value="<?php echo change_dateformat_reverse_db($array_reward['start_date']); ?>" autocomplete="off"></td>
  </tr>
  <tr>
    <td class="text1" align="right">Expiry Date <font color="#FF0000">*</font>:</td>
    <td class="normaltext"><input name="end_date" id="end_date" type="text" class="login_ipboxin"  readonly="readonly" value="<?php echo change_dateformat_reverse_db($array_reward['end_date']); ?>" autocomplete="off"></td>
  </tr>
  <tr>
    ...
    <select name="status" id="status" class="login_ipboxin" style="height:28px; width:286px;">
        ...
        <option value="1"<?php if($array_reward['status'] == '1') { ?> selected="selected" <?php } ?>>Active</option>
        <option value="0"<?php if($array_reward['status'] == '0') { ?> selected="selected" <?php } ?>>Inactive</option>
    </select></td>
  </tr>
  ...
  <tr>
    ...
    <td class="normaltext"><input name="amount" id="amount" type="text" class="login_ipboxin"  value="<?php echo $array_reward['price']; ?>" ></td>
  </tr>
  ...
  <tr>
    ...
    <td class="normaltext"><input name="submit" type="submit" value="Update" class="normalbutton"></td>
  </tr>
</table>
</form>
</div>
