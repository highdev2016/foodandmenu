...
function main()
{
	...
	{
			mysql_query("update  restaurant_menu_category set category_name='".mysql_real_escape_string($_REQUEST['category_name'])."' where id='".$_REQUEST['hid']."'");		
			$errors="Category has been updated successfully ";
	}
	
	if($_REQUEST['id']!="")
	{
		$category_name=mysql_fetch_array(mysql_query("select * from restaurant_menu_category where id='".$_REQUEST['id']."'"));
	}
?>
...
function cancel_it()
{
	window.location.href="manage_menu_category.php?page=<?php echo $_REQUEST['page']?>";
}
</script>
...
<form name="frmchangpass" action="" method="post" onsubmit="return validate()">
  ...
  <tr>
    ...
    <td class="normaltext"><p>
      <input name="submit" type="submit" value="Edit" class="normalbutton" onClick="return checkpassword();">
