...
if($_REQUEST['btn_submit']!="")
{
	...
	if($_REQUEST['review_hid']!="" && $sql_count>0)
	{
		$sql_customer = mysql_fetch_array(mysql_query("SELECT * FROM restaurant_customer WHERE id = '".$_SESSION['customer_id']."'"));
		
		
		
			$sql_update_review = ("update restaurant_reviews set post_date='".change_dateformat($_REQUEST['post_date'])."',customer_name='".htmlspecialchars(stripslashes($_REQUEST['customer_name']),ENT_QUOTES)."', restaurant_name = '".htmlspecialchars(stripslashes($sql_restaurant_name['restaurant_name']),ENT_QUOTES)."', customer_review='".htmlspecialchars(stripslashes($_REQUEST['review']),ENT_QUOTES)."' ");
			
			if($image_review != ''){
				$sql_update_review.=" , review_picture = '".$image_review."'";
			}
			
			$sql_update_review.="  where id='".$_REQUEST['review_hid']."'";
			
			$query_review = mysql_query($sql_update_review);
			
			
			
		if($image_review != ''){
			mysql_query("INSERT INTO restaurant_photo SET image_name = '".$image_review."' , restaurant_id = '".$_REQUEST['id']."'");
		}
		
		$sql_select_rating = mysql_fetch_array(mysql_query("SELECT * FROM restaurant_rating WHERE review_id = '".$_REQUEST['review_hid']."'"));
		if($_REQUEST['rate_count']!=''){
		if($sql_select_rating['score_1'] == 1){
			$sql_update_rating  = mysql_query("UPDATE restaurant_basic_info SET rated = rated - 1 WHERE id = '".$sql_select_rating['restaurant_id']."'");
		}
		else if($sql_select_rating['score_2'] == 1){
			$sql_update_rating  = mysql_query("UPDATE restaurant_basic_info SET rated = rated - 2 WHERE id = '".$sql_select_rating['restaurant_id']."'");
		}
		else if($sql_select_rating['score_3'] == 1){
			$sql_update_rating  = mysql_query("UPDATE restaurant_basic_info SET rated = rated - 3 WHERE id = '".$sql_select_rating['restaurant_id']."'");
		}
		else if($sql_select_rating['score_4'] == 1){
			$sql_update_rating  = mysql_query("UPDATE restaurant_basic_info SET rated = rated - 4 WHERE id = '".$sql_select_rating['restaurant_id']."'");
		}
		else if($sql_select_rating['score_5'] == 1){
			$sql_update_rating  = mysql_query("UPDATE restaurant_basic_info SET rated = rated - 5 WHERE id = '".$sql_select_rating['restaurant_id']."'");
		}
		//$sql_update = mysql_query("UPDATE restaurant_basic_info SET rated = rated + ".$_REQUEST['rate_count']." WHERE id = '".$sql_select_rating['restaurant_id']."'");
		}
		
		$rating_status = updaterate($_REQUEST['rate_count'],$_REQUEST['rating_hid']);
...
<input name="review_picture" type="file" class="restaurant_browse" />
...
<textarea name="review" id="review" cols="" rows="" class="restaurant_browse2"><?php if($res_reviews['abuse_status']==1 && $res_reviews['status']==0){?><?php } else{ echo $res_reviews['customer_review']; }?></textarea>
