...
if($_REQUEST['sub'] == 'sub'){
	...
	if($_SESSION['coupon_code'.$ses_rest_id]!=''){
		...
		if($num_rows > 0)
		{
			...
			{
				$_SESSION['coupon_discount'.$ses_rest_id] = ($sql_sel_coupon['discount']*$cart_amt)/100;
			}
			else
			{
				$_SESSION['coupon_discount'.$ses_rest_id] = ($cart_amt - $sql_sel_coupon['coupon_price']);
			}
		}
		else
		{
                ...
                <div class="food_content">
                    ...
                    <h2 class="popular_item"><a href="#top_div" onClick="popularitem()">More Business Info... </a></h2>
                        ...
                        <table width="100%" border="0" cellspacing="1" class="restro2_table">
                          ...
                          <?php 
						  $amount = $amount + ($array_items['price']*$array_items['quantity']) - $_SESSION['coupon_discount'.$ses_rest_id];
						...
						$j.ajax({
							...
							success : function(data, textStatus, jqXHR){
								...
								if(data != 'Invalid Coupon Code.')
								{
									...
									$j("#coup_code").html(' <a href ="javascript:void(0);" onclick="change_coupon()">Change</a>   <a href ="remove_coupon.php?id='+id+'" onclick="return confirm('+cn_msg+');" >Remove Coupon');
								}
								else
								{
                    ...
                    Coupon Code : <span id="code"></span><span id="coup_code"></span>
