...
function add_menu_spl_ins(id,menu_id){
	$.ajax({
		url : 'add_menu_spl_ins_div.php',
		type : 'POST',
		...
		success : function( data, textStatus, jqXHR){
			...
			var menuContent = data;
			var menuDiv = document.createElement('div');
			var menuDivid = menuDiv.id = 'menu_spl_ins_div_' + id+'_'+menu_id;
			//menuDiv.setAttribute("class","mainmanu");
			menuDiv.innerHTML = menuContent;
			document.getElementById('spl_ins_new_div'+menu_id).appendChild(menuDiv);
			document.getElementById('menu_spl_ins_id_'+menu_id).value=parseInt(id)+1;
		},
		/*complete : function( jqXHR, textStatus){
			alert(3);
		},*/
...
function remove_menu_spl_ins_div(id,menu_id){
	cnfm = confirm('Are you sure?');
	if(cnfm == true)
	{
		$("#menu_spl_ins_div_"+id+"_"+menu_id).remove();	
	}
}

function add_menu_spl_ins_option(id,menu_id,spl_ins_id){
	$.ajax({
		url : 'add_menu_spl_ins_option_div.php',
		type : 'POST',
		data : 'id=' + id+'&menu_id='+menu_id+'&spl_ins_id='+spl_ins_id,
		//dataType : 'json',
		beforeSend : function(jqXHR, settings ){
			//alert(1);
		},
		success : function( data, textStatus, jqXHR){
			...
			var menuContent = data;
			var menuDiv = document.createElement('div');
			var menuDivid = menuDiv.id = 'menu_spl_ins_option_div_' + id+'_'+menu_id+'_'+spl_ins_id;
			//menuDiv.setAttribute("class","mainmanu");
			menuDiv.innerHTML = menuContent;
			document.getElementById('menu_spl_ins_option_'+menu_id+'_'+id).appendChild(menuDiv);
			document.getElementById('menu_spl_ins_option_id_'+menu_id+'_'+id).value=parseInt(id)+1;
		},
		/*complete : function( jqXHR, textStatus){
			alert(3);
		},*/
	...
	?>
        ...
        if($subcat_num_rows_new > 0)
		{
			...
			?>
                ...
                <a href="javascript:void(0);" class="used_cls" style="float:left" onClick="add_menu_spl_ins(document.getElementById('menu_spl_ins_id_<?php echo $row_item['id']; ?>').value,'<?php echo $row_item['id']; ?>')">Add Special Instruction</a>
                <a href="javascript:void(0);" class="used_cls" style="float:right">Copy Special Instruction</a>
                <div class="close close-new" onClick="close_spl_ins_div('<?php echo $row_item['id']; ?>');"><a href = "javascript:void(0);"> </a> </div>
                <div class="l-contnt up-contnt" id="spl_ins_new_div<?php echo $row_item['id']; ?>"> 
                <form name="spl_ins" id="spl_ins" action="" method="post" enctype="multipart/form-data">
                <input type="hidden" name="menu_spl_ins_id_<?php echo $row_item['id']; ?>" id="menu_spl_ins_id_<?php echo $row_item['id']; ?>" value="1">
                <input type="hidden" name="countsplmenu_<?php echo $row_item['id']; ?>[]" id="countsplmenu" value="1">
				...
				?>
                    ...
                    <div class="clear"></div>
                    <div style="border-bottom::1px solid dashed #eee;" id="spl_ins_menu_<?php echo $row_item['id']; ?>">
                    Special Instruction : <input type="text" class="restaurant" name="spl_ins_txt_<?php echo $row_item['id']; ?>[]" id="spl_ins_txt" value="<?php echo $row_ins['special_instruction']; ?>">
                    ...
                    <tr>
                     <td>
                      Title : <input type="text" class="restaurant" name="ins_title_<?php echo $row_item['id']; ?>[]" id="ins_title" value="<?php echo $row_ins_item['title']; ?>" />
                     </td>
                      <td>
                     Price : <input type="text" class="restaurant" name="ins_price_<?php echo $row_item['id']; ?>[]" id="ins_price" value="<?php echo $row_ins_item['price']; ?>" />
                      </td>
                     <td>
