...
<?php
while ($row_search = mysql_fetch_array($res_search)) {
	$res_add = $row_search['restaurant_address']." ".$row_search['restaurant_city']." ".$row_search['restaurant_state']." ".$row_search['restaurant_zipcode']." ".$row_search['restaurant_country'];
    $distance1 = distance($_SESSION['address'], $res_add);
    if ($distance1 <= $search_distance) {
        ?>
