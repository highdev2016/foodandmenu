...
$sql = "SELECT * FROM restaurant_reviews as rr,restaurant_rating as rrat WHERE rr.id = rrat.review_id AND rr.customer_id='".$customer_id."' AND rrat.customer_id='".$customer_id."' AND rr.status=1 AND rr.review_status = 0 AND rr.review_status = 0 order by rr.score ".$sort_type." LIMIT 0,5";

//"SELECT SUM(score_3*3 + score_5*5) as score FROM `restaurant_reviews` as rr , restaurant_rating as rrat WHERE rr.id = rrat.review_id AND rrat.customer_id = 84 AND rr.customer_id = 84";

//$sql = "SELECT * , SUM(score_3*3 + score_5*5) as score  FROM restaurant_reviews as rr,restaurant_rating as rrat WHERE rr.id = rrat.review_id AND rr.customer_id='".$customer_id."' AND rrat.customer_id='".$customer_id."' AND rr.status=1  order by id ".$sort_type." LIMIT 0,5";

$sql_res = mysql_query($sql);

$count = mysql_num_rows($sql_res);
$count11 = mysql_num_rows(mysql_query("SELECT * FROM restaurant_reviews as rr,restaurant_rating as rrat WHERE rr.id = rrat.review_id AND rr.customer_id='".$customer_id."' AND rrat.customer_id='".$customer_id."' AND rr.status=1 AND rr.review_status = 0 AND rr.review_status = 0 order by rr.score ".$sort_type." "));
...
while($row=mysql_fetch_array($sql_res)) 
{
	...
	$sql_updated_review = mysql_query("SELECT * FROM restaurant_reviews as rr,restaurant_rating as rrat WHERE rr.id = rrat.review_id AND rr.customer_id='".$customer_id."' AND rrat.customer_id='".$customer_id."' AND rr.status=1 AND parent_id = '".$row['id']."' AND rr.review_status = 1 ORDER BY id DESC");
	...
	$html_data.='<div class="restu-block">
		...
		$html_data.='<p>
			...
			$html_data.='<span class="like_text" id="like_count_'.$row_updated_review['id'].'">'. get_like_unlike_count_on_review($row['id'], 'like').'</span><img src="images/like_select.png" width="16" height="16" />';
							 } else { 
			$html_data.='<span class="like_text" id="like_count_'.$row_updated_review['id'].'">'.get_like_unlike_count_on_review($row_updated_review['id'], 'like').'</span><a href="like" id="do_like_'.$row['id'].'" class="like" review="'.$row_updated_review['id'].'" cid="'.$_SESSION['customer_id'].'"><img src="images/like.png" width="16" height="16" /></a>';
							}
			$html_data.='</li>
			...
			$html_data.='<a href="dislike" id="do_dislike_'.$row_updated_review['id'].'" class="dislike" review="'. $row_updated_review['id'].'" cid="'.$_SESSION['customer_id'].'"><img src="images/unlike.png" width="16" height="16" /></a><span class="like_text2" id="dislike_count_'.$row_updated_review['id'].'">'.get_like_unlike_count_on_review($row['id'], 'dislike').'</span>';
			 } 
			$html_data.='</li>';
			 } else {
			$html_data.='<li id="li_like_'.$row_updated_review['id'].'"><span class="like_text" id="like_count_'.$row['id'].'">'.get_like_unlike_count_on_review($row_updated_review['id'], 'like').'</span><a href="like" onClick="like_unlike_alert("Please Login First to Like this"); return false;"><img src="images/like_select.png" width="16" height="16" /></a></li>
			...
			$html_data.='</ul></div>';
				...
				$html_data.='<div class="report-ab">  
							<a href="mailto:?body='.$row_updated_review['customer_review'].'&subject='.date("d-m-Y", strtotime($row_updated_review['post_date'])).'  By '.$row_updated_review['customer_name'].'" target="_blank">Email</a>';
				...
				$html_data.='<div class="report-ab">
					...
					$html_data.='<a href="login.php" style="color:#000000;" onClick="return confirm('.$confirm.');">Report Abuse</a>';
					 } 
				$html_data.='</div>';
				 } 
