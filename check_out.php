...
if(($_REQUEST['reorder'] == 1) && ($_REQUEST['type']!='') && ($_REQUEST['order_id']!='')){
	...
	if($_REQUEST['payment_mode'] == "credit_card")
	{
		$sql_insert_order.=" , card_type = '".$_REQUEST['card_type']."', card_no = '".$_REQUEST['card_no']."', cvv_no =  '".$_REQUEST['cvv_no']."', expiry_date = '".$_REQUEST['exp_month']."-".$_REQUEST['exp_year']."'";
	}
	...
	$commission = (($_SESSION['del_charge'.$ses_rest_id.'_'.$_SESSION['group_order_details_id'.$ses_rest_id]]+$_REQUEST['amount2'] + $_REQUEST['tax'] + $_REQUEST['tip'] + $service_fee)*$sql_del_com_info['commission'])/100;
		...
		$sql_insert_order.=" ,delivery_charge = '".$_REQUEST['delivery_charge']."', price_with_del_charge = '".($_REQUEST['delivery_charge']+$_REQUEST['amount2'] + $_REQUEST['tax'] + $_REQUEST['tip'] + $service_fee) ."' , service_fee = '".$service_fee."'";
	...
	exit;
        ...
        try {
            ...
            $charge = \Stripe\Charge::create(array(
                    ...
                    "currency" => "usd",
                    "source" => $token,
                    "description" => (($_REQUEST['type'] == 'del') ? 'Online Delivery' : 'Online Pickup')
                )
            );
...
<p style="font:bold 12px Arial, Helvetica, sans-serif; color:#000000; margin:0 0 4px 15px;">Expiration Date : '.$_REQUEST['exp_month']."-".$_REQUEST['exp_year'].'&nbsp;</p>';
            ...
            // Card was declined.
            $errors['error_type'] = 'InvalidRequest';
            $errors['error_message'] = $e->getJsonBody()['error']['message'];
...
<script type="text/javascript">
    <?php if (count($errors) > 0) : ?>
        <?php if ($errors['error_type'] == 'InvalidRequest') : ?>
        alert("<?php echo $errors['error_message']?>");
        <?php endif ?>
    <?php endif ?>
$(function() {
...
function check_it()
{
    ...
    $('#address_ship').val('address1');
    $('#apart').val('home');
    $('#city').val('city1');
    $('#state').val('state1');
    $('#zipcode').val('12345');

    $('#billing_address').val('address2');
    $('#apart_billing').val('home');
    $('#billing_city').val('city2');
    $('#billing_state').val('state2');
    $('#billing_zip').val('12345');
    $('#card_no').val('4242424242424242');
    $('#cvv_no').val('111');
    $('#exp_month').val('08');
    $('#exp_year').val('17');*/
...
function stripeResponseHandler(status, response) {
    ...
    if (response.error) {
        ...
        f.append("<input type='hidden' name='place_order' value='Place My Order' />");
        document.frm.buttonclick.value="sale";
...
function check_shipping_billing(){
	...
	if($j("#check1").prop("checked") == true){
		$j("#billing_address").val($j("#address_ship").val());
		$j("#billing_city").val($j("#city").val());
		$j("#billing_state").val($j("#state").val());
		$j("#billing_zip").val($j("#zipcode").val());
		$j("#apart_billing").val($j("#apart").val());
		show_div_billing($j("#apart").val())
		$j("#information_billing").val($j("#information").val());
		$j("#apt_name_billing").val($j("#apt_name").val());
		$j("#apt_no_billing").val($j("#apt_no").val());
		$j("#gate_code_billing").val($j("#gate_code").val());
	}else{
		...
		$j("#billing_zip").val('');
		$j("#apart_billing").val('');
		$j("#information_billing").val('');
		$j("#apt_name_billing").val('');
		$j("#apt_no_billing").val('');
		$j("#gate_code_billing").val('');
	}
}
</script>
...
<img src="images/ajax-loader.gif" align="middle"/> <br>
...
<?php if($sql_restaurant['phone'] != "") { ?>
<p style="color:#000000; font-size:16px;">
<?php echo "Phone # : ".$sql_restaurant['phone'];?></p>
<?php } ?>
...
<div class="checkout_right_panel">
    ...
    <?php } ?>
        ...
        <div class="check_form_section">
        	...
        	<div class="check_form_box check_new_form">
            ...
            <div class="check_form_box check_new_form">
                ...
                <div class="check_form">
                    <p>Address*</p>
                    <input name="address" type="text" id="address_ship"  value="<?php echo $sql_customer_details['address']; ?>" />
                </div>
                <div class="check_form">
                <p>Home/Apartment * : </p>
                <select name="apart" id="apart" class="sel_cls" onChange="show_div(this.value);">
                <option value="">---Select---</option>
                <option value="home"<?php if($sql_customer_details['home_apartment'] == "home") { ?> selected="selected" <?php } ?>>Home</option>
                <option value="apartment"<?php if($sql_customer_details['home_apartment'] == "apartment") { ?> selected="selected" <?php } ?>>Apartment</option>
                <option value="others"<?php if($sql_customer_details['home_apartment'] == "others") { ?> selected="selected" <?php } ?>>Others</option>
                </select>
                </div>
                <?php
				if($sql_customer_details['home_apartment'] == "others")
				{
					$display_other = 'block';
				}
				else
				...
				if($sql_customer_details['home_apartment'] == "apartment")
				{
					$display_apt = 'block';
				}
				else
                ...
                <div class="check_form" id="apt_div" style="display:<?php echo $display_apt; ?>">
                <p>Apartment Name </p>
                <input name="apt_name" type="text" id="apt_name" class="regifield"  value="<?php echo $sql_customer_details['apt_name']?>" />
                <div class="clear"></div>
                
                <div class="first_name">
                <p>Apartment Number</p>                                     
                <input name="apt_no" type="text" id="apt_no" class="regifield"  value="<?php echo $sql_customer_details['apt_no']?>"  /> 
                </div>
                
                <div class="first_name last_nme">
                <p>Gate Code</p>
                <input name="gate_code" type="text" id="gate_code" class="regifield"  value="<?php echo $sql_customer_details['gate_code']?>"  />
                </div>
                </div>
                <div class="check_form" id="info_div" style="display:<?php echo $display_other; ?>">
                <p>Information </p>
                <input name="information" type="information" id="information" class="regifield"  value="<?php echo $sql_customer_details['information']?>"  />
                </div>
            ...
            <div class="check_form_box check_new_form">
                ...
                <div class="check_form">
                <p>Home/Apartment * : </p>
                <select name="apart_billing" id="apart_billing" class="sel_cls" onChange="show_div_billing(this.value);">
                <option value="">---Select---</option>
                <option value="home"<?php if($sql_customer_details['billing_home_apartment'] == "home") { ?> selected="selected" <?php } ?>>Home</option>
                <option value="apartment"<?php if($sql_customer_details['billing_home_apartment'] == "apartment") { ?> selected="selected" <?php } ?>>Apartment</option>
                <option value="others"<?php if($sql_customer_details['billing_home_apartment'] == "others") { ?> selected="selected" <?php } ?>>Others</option>
                </select>
                </div>
                <?php
				if($sql_customer_details['billing_home_apartment'] == "others")
				{
					$display_other = 'block';
				}
				else
				...
				if($sql_customer_details['billing_home_apartment'] == "apartment")
				{
					$display_apt = 'block';
				}
				else
                ...
                <div class="check_form" id="apt_div_billing" style="display:<?php echo $display_apt; ?>">
                <p>Apartment Name </p>
                <input name="apt_name_billing" type="text" id="apt_name_billing" class="regifield"  value="<?php echo $sql_customer_details['billing_apt_name']?>" />
                <div class="clear"></div>
                
                <div class="first_name">
                <p>Apartment Number</p>                                     
                <input name="apt_no_billing" type="text" id="apt_no_billing" class="regifield"  value="<?php echo $sql_customer_details['billing_apt_no']?>"   /> 
                </div>
                
                <div class="first_name last_nme">
                <p>Gate Code</p>
                <input name="gate_code_billing" type="text" id="gate_code_billing" class="regifield"  value="<?php echo $sql_customer_details['billing_gate_code']?>"  />
                </div>
                </div>
                <div class="check_form" id="info_div_billing" style="display:<?php echo $display_other; ?>">
                <p>Information </p>
                <input name="information_billing" type="text" id="information_billing" class="regifield"  value="<?php echo $sql_customer_details['billing_information']?>"  />
                </div>
                <div class="check_form">
                    <p>City*</p>
            ...
            <div id="credit_div" style="display:none;" class="payment_div">
		...
		$tax = round($tax1, 2);
		 ?>
        <fieldset class="special_instructions">
        <legend>Total $<input type="text" name="amount" id="amount" value="<?php echo ($total_amount + $tax + $service_fee); ?>" style="border:none; float:right; color:#FA8730; font: bold 23px calibri; margin: 0 413px 0 0;" readonly></legend>
        ...
        <input type="hidden" name="delivery_charge" id="delivery_charge" value="<?php echo $_SESSION['del_charge'.$ses_rest_id];?>">
...
function show_div_billing(val)
{
  ...
  if(val == "others")
  {
	  $j("#apt_name_billing").val('');
	  $j("#apt_no_billing").val('');
	  $j("#apt_div_billing").slideUp(1000);
	  $j("#info_div_billing").slideDown(1000);
	  
  }
  if(val == "apartment")
  {
	  $j("#information_billing").val('');
	  $j("#info_div_billing").slideUp(1000);
	  $j("#apt_div_billing").slideDown(1000);
  }
  if(val == "home")
  {
	  $j("#apt_name_billing").val('');
