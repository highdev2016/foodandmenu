...
if($_REQUEST['action']=='del' && $_REQUEST['del_coupon_id']!=''){
	...
	header("location:edit_special_offer.php?restaurant_edit_id=".$_REQUEST['restaurant_edit_id']."&success=2");
	exit();
}
if($_REQUEST['action']=='del' && $_REQUEST['del_deal_id']!=''){
	mysql_query("delete from restaurant_deals where id='".$_REQUEST['del_deal_id']."'");
	header("location:edit_special_offer.php?restaurant_edit_id=".$_REQUEST['restaurant_edit_id']."&success=4");
	exit();
}
...
if(count($_POST)>0 && $_REQUEST['submit']=="Save & Continue")
{
	...
	header("location:edit_confirmation.php?restaurant_edit_id=".$_REQUEST['restaurant_edit_id']."");
	
	}
	else{
		header("location:edit_confirmation.php?restaurant_edit_id=".$_REQUEST['restaurant_edit_id']."&status=error");
	}
}
?>
...
<table width="98%" border="1" bordercolor="c5c8d5" cellspacing="0" cellpadding="0" style="border-collapse:collapse; margin-top:10px;" align="center">
  ...
  <tr>
    ...
    <td class="all_restaurant2"><a href="edit_daily_coupon.php?coupon_id=<?php echo $array_deals['id']?>&restaurant_edit_id=<?php echo $_REQUEST['restaurant_edit_id']?>" class="all_edit">Edit</a>&nbsp;<a href="edit_special_offer.php?action=del&del_coupon_id=<?php echo $array_deals['id']?>&restaurant_edit_id=<?php echo $_REQUEST['restaurant_edit_id']?>" class="all_edit" onClick="return confirm('Are you sure you want to delete this coupon deals?');">Delete</a></td>
  </tr>
<?php } ?>
</table></div>
<?php
