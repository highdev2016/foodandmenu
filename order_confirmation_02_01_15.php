...
<style type="text/css">
.menu_container_email p { text-align:center; padding-top:18px; text-transform:uppercase; font-size:18px; color:#fff;}
.reprint { padding:0 100px; text-transform:uppercase; font-size:16px;}
.reprint fieldset { border-bottom:0; border-left:0; border-right:0; border-top:2px dashed #009; padding-left:200px; text-align:center; width:300px; margin:0 auto 15px auto;}
.reprint fieldset legend { padding:0 10px;}
.reprint fieldset legend a { font-size:18px; text-decoration:none; color:#000;}
.email_logo{ float:left; width:216px;}
.future_pickup {}
.new_email_content { padding:10px;}
.future_pickup { width:200px; float:right; text-align:center;}
.future_pickup p { text-align:center; padding-top:5px;}
.future_pickup p span{ font-weight:bold;}
.pick_up { font-size:20px;}
.table_content table { border-collapse:collapse;}
.table_content table td { padding:0 5px; border:1px solid #ccc;}
.bottom_future { text-align:center;}
.coinfirm_text { border:1px solid #C5C8D5;}
.bottom_future h2 { font-size:20px; padding:10px 0;}
.coinfirm_text h4 { font-weight:bold; font-size:18px;}
</style>
...
<body style="background:url(images/body_bg.png) center top no-repeat;">
...
<div class="new_email_content">
    ...
    <div class="email_logo"><a href="https://foodandmenu.com"><img src="images/logo.png" /></a></div>
    <div class="future_pickup">
    <p style="color:#000000; font-size:14px;"> Order No : <span> <?php echo 'OR-00'.$order_id; ?>  </span></p>
    <p style="color:#000000; font-size:14px; width:210px;">Order Placed on <?php echo date("m-d-Y", strtotime($sql_menu['order_date'])); ?> 
    <?php $time = substr($sql_menu['order_date'],-8); ?>
    <?php echo date("g:i A", strtotime($time)); ?>
    </p>
    <p style="color:#000000; font-size:14px;">for <span class="pick_up"> <?php if($sql_menu['type'] == 'del'){ echo "Delivery"; }else{ echo "Pick Up"; }?></span></p>
  ...
  <div class="email_logo email_logo2" style="border-right:1px solid #000000; margin:5px 0 5px 0; color:#000000;">
    ...
    <p style="padding-top:5px; color:#000000; font-size:15px;"><strong>Address <span>:</span></strong> <span> <?php echo $sql_contact_details['address']."<br /> ".$sql_contact_details['city']." ".$sql_contact_details['state']." ".$sql_contact_details['zipcode']; ?></span></div>
   
   
    <div class="future_pickup  pickup_new" style="float:left;">
    ...
    <?php if(!empty($sql_contact_details['special_ins'])){ ?>
    	...
    	<p style="text-align:left; margin-left:5px; color:#000000; font-size:15px;"><?php echo $sql_contact_details['special_ins']; ?></p>
    <?php } ?>
  ...
  <div class="table_content">
    ...
    <table width="100%" border="1" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td width="14%" align="right" colspan="2" >Price</td>
      </tr>
        ...
        <tr>
            ...
            ?></td>
            <td align="right" colspan="2">$ <?php echo $sub_total_amount = ($array_select['quantity']*$array_select['menu_price'] + $ins_price)."<br>"; ?></td>
        </tr>
        <?php $i++;
      ...
      <tr>
        <td width="60%" rowspan="8" align="center" valign="middle" colspan="2">
        
        <?php if($sql_menu_order_details['payment_mode'] == 'cash'){?>
        		<span class="cash_order" style="border: 1px solid rgb(0, 0, 0); padding: 20px;">Will Pay With Cash</span>
        <?php }else{ ?>
        		<span class="cash_order" style="border: 1px solid rgb(0, 0, 0); padding: 20px;">Do not Charge  - 
                PREPAID
                </span>
		<?php }?>
      ...
      <tr>
        ...
        <td align="right"><?php echo "$ ".($sub_total-($sql_menu_order_details['coupon_discount'] + $sql_menu_order_details['reward_points'])); ?></td>
      </tr>
      ...
      <?php } ?>
      <?php if($sql_menu_order_details['service_fee']!=''){ ?>
      <tr>
      ...
      <tr>
        ...
        <td align="right"><span><?php echo "$ ".$total_amount; ?></span></td>
      </tr>
    </table>
 ...
 <?php
	if($sql_menu_order_details['coupon_code'] != "")
	 	{
			?>
         <div class="coinfirm_text"><h4>Coupon Name : <?php echo getNameTable("restaurant_coupon","coupon_name","coupon_code",$sql_menu_order_details['coupon_code']); ?></h4></div>
 		 <?php /*?><div class="coinfirm_text"><h4>Coupon Code : <?php echo $sql_menu_order_details['coupon_code']; ?></h4></div><?php */?>
		...
		{
			?>
            <div class="coinfirm_text"><h4>Coupon Name : <?php echo "N/A"; ?></h4></div>
            <?php /*?><div class="coinfirm_text"><h4>Coupon Code : <?php echo "N/A"; ?></h4></div><?php */?>
            <?php
		}
	?>
                        ...
                        <small> <img src="images/downld.png" alt="" align="absmiddle" /> Received: <?php echo date("h:i A", strtotime($sql_menu['order_date'])); ?></small>
                         <div class="clear"></div>
                            ...
                            <?php
								...
								{
									echo "<a href='javascript:void(0);' id='sel1' onclick='get_sel_time(".$sql_bus_del_info['del_time_slot1'].",1);'>".$sql_bus_del_info['del_time_slot1']." mins</a> &nbsp;";
									echo "<a href='javascript:void(0);' id='sel2' onclick='get_sel_time(".$sql_bus_del_info['del_time_slot2'].",2);'>".$sql_bus_del_info['del_time_slot2']." mins</a> &nbsp;";
									echo "<a href='javascript:void(0);' id='sel3' onclick='get_sel_time(".$sql_bus_del_info['del_time_slot3'].",3);'>".$sql_bus_del_info['del_time_slot3']." mins</a> <br>";
									echo "<a href='javascript:void(0);' id='sel4' onclick='get_sel_time(".$sql_bus_del_info['del_time_slot4'].",4);'>".$sql_bus_del_info['del_time_slot4']." mins</a> &nbsp;";
									echo "<a href='javascript:void(0);' id='sel5' onclick='get_sel_time(".$sql_bus_del_info['del_time_slot5'].",5);'>".$sql_bus_del_info['del_time_slot5']." mins</a> &nbsp;";
									echo "<a href='javascript:void(0);' id='sel6' onclick='get_sel_time(".$sql_bus_del_info['del_time_slot6'].",6);'>".$sql_bus_del_info['del_time_slot6']." mins</a> &nbsp;";
								}
								else
								{
									echo "<a href='javascript:void(0);' id='sel7' onclick='get_sel_time(".$sql_bus_del_info['pickup_time_slot1'].",7);'>".$sql_bus_del_info['pickup_time_slot1']." mins</a> &nbsp;";
									echo "<a href='javascript:void(0);' id='sel8' onclick='get_sel_time(".$sql_bus_del_info['pickup_time_slot2'].",8);'>".$sql_bus_del_info['pickup_time_slot2']." mins</a> &nbsp;";
									echo "<a href='javascript:void(0);' id='sel9' onclick='get_sel_time(".$sql_bus_del_info['pickup_time_slot3'].",9);'>".$sql_bus_del_info['pickup_time_slot3']." mins</a> <br>";
									echo "<a href='javascript:void(0);' id='sel10' onclick='get_sel_time(".$sql_bus_del_info['pickup_time_slot4'].",10);'>".$sql_bus_del_info['pickup_time_slot4']." mins</a> &nbsp;";
									echo "<a href='javascript:void(0);' id='sel11' onclick='get_sel_time(".$sql_bus_del_info['pickup_time_slot5'].",11);'>".$sql_bus_del_info['pickup_time_slot5']." mins</a> &nbsp;";
									echo "<a href='javascript:void(0);' id='sel12' onclick='get_sel_time(".$sql_bus_del_info['pickup_time_slot6'].",12);'>".$sql_bus_del_info['pickup_time_slot6']." mins</a> &nbsp;";
								}
